 ...
 *
 * Runs inside an Electron `utilityProcess`. Deliberately thin: it builds the
 * registry, wraps `process.parentPort` as a channel, and hands both to
 * `AgentHostServer`. All the behavior is there and is tested in-process over an
 * in-memory channel.
 *
 * This process owns agent loops and tool execution. It does **not** own the
 * event log, session state, or policy — those stay in main, which keeps one
 * append-only writer per session (§5.1) and one place where every permission
 * decision is recorded (§13).
 */
...
if (port === undefined) {
  throw new Error('agent host must run as an Electron utilityProcess');
}
...
new AgentHostServer(new ParentPortChannel(port), buildHostRegistry());
