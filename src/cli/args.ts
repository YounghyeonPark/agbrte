...
export function parse(argv: string[]): Parsed {
  ...
  // `loom /srv/api` and `loom attach /srv/api` both work: a first argument that
  ...
  // `loom run "prompt"` with no path means the current directory, so `run` has
  ...
  // arguments, so `loom run add a test` arrived as three of them and "add"
