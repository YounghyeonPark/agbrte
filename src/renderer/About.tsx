...
export function About({ inset = false }: { inset?: boolean } = {}): JSX.Element {
  ...
  if (info === null) {
    ...
    return <div data-testid="about" className={inset ? '' : 'm-auto'} />;
  }

  return (
    /* `inset` is what lets this be a section of Settings as well as a pane of
       its own: the outer centring and padding belong to a destination, and a
       section inside one would inherit them twice. Nothing else differs, so the
       two cannot drift into showing different truths about one installation. */
    <div
      className={inset ? 'grid gap-4' : 'm-auto grid w-full max-w-xl gap-6 p-6'}
      data-testid="about"
    >
      {!inset && (
