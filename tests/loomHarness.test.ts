/**
 * `LoomHarness` — our own agent loop (DESIGN.md §3.7).
 *
...
import { LoomHarnessRuntime, degradeSchema } from '@main/runtime/runtimes/loomHarness.js';
import {
  newAgentId,
  type AgentSpec,
  type ModelEndpoint,
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'loom-harness-'));
...
async function run(
  ...
  const runtime = new LoomHarnessRuntime({
    provider,
    endpoint: ENDPOINT,
...
describe('LoomHarness — basics', () => {
  ...
  it('refuses a spec with no model', async () => {
    const runtime = new LoomHarnessRuntime({ provider: new StubProvider([{}]), endpoint: ENDPOINT });
...
describe('LoomHarness — loop control', () => {
...
describe('LoomHarness — context assembly', () => {
  it('replays a rehydrated seed as ordinary conversation', async () => {
    const provider = new StubProvider([{}]);
    const runtime = new LoomHarnessRuntime({ provider, endpoint: ENDPOINT });
