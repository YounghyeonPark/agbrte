...
| 4 | Multi-session + dashboard | 5th | done — dashboard, Needs-you rail, stall detection, parking, notifications, QuotaScheduler, inbox. **The per-host concurrency cap §8 specifies is not built** |
...
Two things that criterion asks for are still unverified. It says *three models and two auth modes*, and the test uses `echo` throughout — the variety is a claim about admission and capability resolution, not about concurrency. And **the per-host concurrency cap does not exist**: §8 specifies `min(8, cores − 2)` with FIFO queueing above it, and nothing implements a cap of any kind, so ten sessions ran because nothing was limiting them rather than because queueing worked. §16's "process overhead at scale" row names that cap as its mitigation.
...
| Process overhead at scale | memory pressure | Quota-group throttles and idle parking of hosts are built. **The per-host cap is not** — §8 specifies `min(8, cores − 2)` with FIFO queueing and nothing implements one, so concurrency is currently bounded by nothing. Ten concurrent sessions were verified to work, which establishes that they are correct rather than that they are safe at scale: workers are 30–50 MB each |
