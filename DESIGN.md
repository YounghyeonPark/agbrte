...
8. **Automatic splitting — recommended: keep approval, and make the grant a budget rather than a mode.** The signals in §4.3 are measurable enough to act on, and always requiring approval means a long overnight run stalls at the moment it most needs to decompose.
    ...
    Unbuilt, and the machinery it needs already exists: `proposeSplit` writes the event and raises `needsAttention` today.
