...
16. **The protocol version check is all-or-nothing, and it disarms the graceful upgrade path.** `SESSION_PROTOCOL_VERSION` is compared for equality at handshake, which is right when a field moves — a mismatch should be refused before a command half-works. §6.7 added two commands and moved nothing, and an older host would have served everything else correctly; instead every running detached host was refused until restarted, over a feature its user may never invoke — and *restarted* understates it. Confirmed on the server: `agbrte stop` could not reach the v1 host it was meant to retire, because the CLI that asks is the CLI that was just upgraded. A bump therefore forces a `kill`, and the polite shutdown §8 exists to provide is unavailable precisely when an upgrade needs it. The fix is a minimum-supported version, or capability negotiation in `welcome` so a client can find out that `blob.put` is unavailable without losing the connection. Not built, because it is a real design and adding it as a side effect of adding a command is how protocols acquire two versioning schemes.
