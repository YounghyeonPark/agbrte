...
11. **`RuntimeDescriptor.requiresModel` is a boolean answering a three-valued question.** Required for `AgbrteHarness`, *optional* for an installed CLI — these take `-m` and choosing one is legitimate — and meaningless for `echo`. Admission rejects any spec carrying a model when `requiresModel` is false, so model selection for installed CLIs is currently unreachable and `modelArgs` was left out of the manifest rather than shipped as code admission guarantees never runs (§3.12). Making it a tri-state touches the IPC contract and the renderer, which is why it was not bundled into the adapter; it is small, and it is the one thing blocking a real capability.
