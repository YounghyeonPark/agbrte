...
The split between the two halves is a cost split, and the numbers are measured against a live Ollama rather than reasoned about. **Self-description is free**: five models described themselves in 85 ms total, one `/api/show` each, no inference, so `models.list` carries a hint per model (v14). **Probing is not**: ~1.2 s per model here, and two real requests at whatever a cloud endpoint charges — so `models.capabilities` (v14) is asked for the one model somebody selected, automatically where the endpoint is free and behind a button where it is not. The asymmetry that makes the cheap half worth having: a server that lists no tool support for a model is a hard no (Ollama refuses the request outright), while a server that lists tool support is only a claim — `qwen3:0.6b`'s own manifest says `['completion','tools','thinking']`. That is exactly why a declared yes is shown as declared and replaced, not confirmed, by the probe.
...
| 7 | Usage split by **cache creation vs cache read**, which are priced differently, plus thinking tokens | ~~One `cachedInputTokens` conflating both~~ — **fixed**: `cacheReadTokens` / `cacheWriteTokens` are separate through the event, the projection and the agent record, and `pricing` takes an optional rate for each. Thinking tokens remain unmodelled | Was going to be, the moment anything priced a turn |
...
- **Context management** — no server-side compaction to lean on, so it compacts at a high-water mark by calling the **same `rehydrate()`** used for moved workspaces (§5.4). The function that reconstructs context after a move is the function that compacts a running session — one code path, exercised constantly, so the durable path can't rot.
  ...
  Split across the seam §17.18 named, because `rehydrate()` needs the store and a runtime is handed no route to the log. `RuntimeContext.compact(budgetTokens)` is the join: the harness decides **when**, from a token count only it can take, and the owner **does** it and writes `agent.compacted`. That is §17.1's rule — the harness may decide, and may not own — applied to context the same way it is applied to tool calls and their gate.
  ...
  Declining is ordinary. The hook is absent on adapters that run their own loop, and returns `null` when the log holds nothing worth carrying; the history stands in both cases. Running out is not silent either: the provider refuses, and `context_overflow` surfaces as needing a person (§4.1).
...
So **a Windows server could not be attached at all**, and the transport row read "a machine over SSH" — a claim broader than the truth. It read "a Linux or macOS machine over SSH" until a Windows machine was available to build against; see below for what it says now and what that cost.
...
**Windows as a target is built, and every layer of it was wrong in a way reading could not have found.** A Windows machine with `sshd` became available, so the second bootstrap exists: a PowerShell probe, a `.zip` Node, a detached launch through WMI, and §6.2's loopback control channel in place of the unix socket. The row reads "a Linux, macOS, or Windows machine over SSH".

Four things that all passed before a real `sshd` carried them:

- **The upload hung forever.** The Windows analogue of `cat > path` was PowerShell copying its raw standard input into the destination. Locally, `cmd.exe` on a real pipe delivers stdin EOF and it was byte-exact; over ssh the remote wrote exactly one 32 KB buffer and blocked, because Win32-OpenSSH does not deliver stdin EOF to a non-tty child. The lock outlived the hang and was the worse half: the stalled shell held an exclusive handle on a half-written bundle and could not be killed without elevation, so every later run failed `EBUSY` on a file nothing appeared to be using. It uses `scp` now — same OpenSSH, its own framed protocol, and a non-zero exit instead of a stall — staged through `.part` so a dead transfer cannot leave a truncated bundle where a working one was.

- **The probe reported a locked bundle as an unreachable machine.** It read the bundle's first line for a version under `$ErrorActionPreference='Stop'`. A bundle mid-upload is locked on Windows for the length of the transfer — a window this program opens itself on every upgrade — so the failure was reachable in normal use, not just after the bug above. An unreadable version now degrades to empty, which re-uploads.
...
**Built**, because §12.1's client capture cannot exist without it: a screen capture happens on the machine with the screen, and for a remote session the blob store is at the far end of an ssh connection. An `ImageBlock` naming a sha the host has never seen is a dangling reference that fails when the model request is *built* — well after the point where the bytes were actually missing.

**It moves the redaction guarantee.** §12.1 states its ordering rule as "the unredacted frame is never written to disk", and once a capture can cross a network that sentence is no longer the whole of it: bytes that reach a remote host have left the machine they were taken on whether or not anyone stored them. So the rule is now stronger — **painting happens on the machine that took the picture**, and there is no command in this protocol that would carry a raw frame instead. That is only affordable because `content/pixels.ts` runs on plain Node; an Electron-only painter would have forced the opposite pipeline, shipping the frame to the host to be painted there, and the credential would have crossed the wire every time.

**A commit is verified, because a content-addressed store is poisonable.** A blob's name is a claim about its contents and every later reader trusts it — including the dedup above, which skips the transfer entirely on a hash it already holds. Bytes stored under a hash they do not hash to would make all of those readers wrong, so the assembled bytes are hashed and compared before the store sees them, and a mismatch drops the staging rather than writing it under either name.
...
**The chunk *is* the rate limit.** Chunks are sent one at a time and awaited, so every gap between them is a point where the host's pushes get the channel. A single 10 MiB message would hold it for the whole write, and no rate limiting *inside* that message would help. Resumption needs no reconnect handling on either side: every reply carries the byte count the host holds, and the client follows that number rather than its own, which is what makes a retried chunk idempotent rather than duplicated.

**"Transfers once" needed a local copy to be true.** Attachments are stored **per session**, so a hit in a sibling session is served by copying on the host rather than by asking the client to send a screenshot that machine already received. Per-session storage is not an oversight: sessions have independent lifetimes, and a shared store would mean deleting one session silently breaks another's transcript — much worse than a duplicated few megabytes. The copy keeps both properties: one transfer over the wire, independent copies on disk. The lookup is reachable only by naming the hash, and in a content-addressed store naming the hash means having held the content.

**Verified against a real machine, not only in memory.** Everything above is exercised over in-memory channels, which cannot answer whether a chunked base64 JSON transfer survives an actual socket forwarded over ssh. Against the tailnet server: 2.16 MB in 9 chunks, 712 ms, and `sha256sum` **on the far end** matching the locally computed hash — two machines agreeing on the bytes rather than the host agreeing with itself. The second attach took 9 ms and sent nothing, and a sibling session resolved the same hash in 22 ms with no transfer, both confirmed by finding the blob in two session directories on the server's disk.

The first attempt proved nothing and looked like it had. The test frame was filled with `(i * 7) & 0xff` — described in the code as noise, actually a ramp — which zlib crushed to 15 KB, so the whole "chunked" transfer fitted in one chunk and the loop under test never ran. Replaced with a xorshift; the frame went from 15 KB to 2.16 MB and the test started testing something.

**The version bump stranded every running host, and that is the check working — but it costs more than a restart.** `SESSION_PROTOCOL_VERSION` went to 2 and the in-repo tests immediately failed against a detached host built from the previous commit — `host speaks v1, this app speaks v2`, refused at handshake exactly as §6.4 intends. Worth recording because this is the first **purely additive** bump: two new commands, no field moved, and an older host would have served every existing command correctly.

Then the deploy showed what that actually means. `agbrte stop` on the upgraded server printed the same sentence and could do nothing: the running host is v1, the freshly installed CLI is v2, and **the tool that would shut it down politely is the one that can no longer speak to it.** So a protocol bump does not cost a restart — it costs a `kill`, and the documented graceful path is unavailable at exactly the moment an upgrade needs it. That is a chicken-and-egg, not an inconvenience, and it is why §17's entry is worth acting on rather than noting. Killing remains safe (the log is the truth and every session reopens from it), and all five sessions on the server listed correctly afterwards.
...
**Built, and the process belongs to the host.** The host already outlives the app on purpose (§6.4), so a preview survives the turn that motivated it, the app closing, and the lid. Stopped when the session ends and when the host does — a preview server outliving the host that started it is a port answering with nothing to explain it and nothing left that knows how to stop it.

**It is the user's command, never the model's.** §3.12's reaping is a real containment property: whatever an agent starts, ends. An API that starts a *persistent* process would **launder** that guarantee if a model could reach it — "run this in the background" becomes possible again by asking us nicely. Two guards. Structurally, `preview.start` is a protocol command and an IPC method and is deliberately absent from the tool registry, so a model cannot ask for it at all. And it is a write, gated on the human client's role, so the phone pinned to `read-only` by §7's policy cannot start processes on a build box — asserted, including that nothing was spawned.
...
**And `detached` had to become POSIX-only for a second, unrelated reason.** A child in a new Windows console writes its output to that console instead of into the pipe — measured, and total: the same command captures `42\n` without the flag and `""` with it. Every preview log would have been empty, and a dev server that died on a syntax error would have died silently, which is the one thing the "report, do not refuse" rule above depends on not happening.

**The log is that rule's other half**, kept as a bounded ring because a dev server left up for a day produces a great deal of nothing in particular and the interesting part is always the end. It is also how the next finding arrived.

**A remote machine may have no Node but ours.** §6.2 unpacks a private runtime under `~/.agbrte/node` precisely so that attaching a host does not change the machine — so the first thing a user types here, `npm run dev`, failed with `/bin/sh: 1: node: not found` on a machine demonstrably running Node at that moment. Found by running it against a real host, and found *through the log*, which is the argument for the log arriving as its own first example. The host's own runtime directory is now **appended** to the command's `PATH`, never prepended: if the machine has its own Node, the user's command should get that one, and shadowing it would be exactly the "we changed your machine" the private runtime exists to avoid.

Verified end to end against the real remote host — started by us, forwarded while still compiling (`reachable=false`, log `compiling…`), reachable on the same local port once it came up, detected by the host, `HTTP 200` through the tunnel with the expected bytes, `SIGTERM` on stop with the port released, and no leftover process.

The commands are `preview.start` / `preview.stop` / `preview.servers` / `preview.log` at protocol **v4**. Not v3: a host deployed at v3 has `preview.ports` and not these, and adding them to v3 would have made `supports()` lie to every client that met one.

**Built, and verified against a real remote host.** A loopback-bound `python3 -m http.server` on a build box, unreachable from this machine by any route except the tunnel, fetched through it with HTTP 200 and the expected bytes; asked twice and answered with the same forward; torn down with the session and gone. `portForwardOut` is the first of §6.2's six booleans to be read by anything.

**Detection reads `/proc/net/tcp`, and shows only this user's ports.** No subprocess and nothing to have installed — a remote host is Node on Linux by construction, and so are the transports queued behind it. The uid filter is the design decision, and the capture that verified the parser is also what argues for it: that machine had twelve listeners under four uids, of which five were ours. Among the other seven was **another user's service on port 4000, bound to `0.0.0.0`**. Offering the whole list turns a preview feature into a reconnaissance one — it tells you what your colleagues are running, on which ports, right now, and it arrives as a helpful dropdown nobody asked for. §17 Q9 already worries about exactly this machine. The bind address is carried through rather than normalised away, because `127.0.0.1` and `0.0.0.0` are both ordinary and the difference is who can reach it.

**A forward that opened is not a forward that works, and that was a real defect.** `ssh -N -L` binds the local end immediately, so the transport's readiness test — "the port answers" — is true for a remote port with nothing behind it. Forwarding port 9 on a live host reported success, and the failure arrived at fetch time as a connection reset; to a user that reads as "the preview is broken" and sends them to debug the wrong machine. Each forward is now probed, and the numbers are measured rather than assumed: **a dead forward closes the probe in 4–5 ms, a live one stays open indefinitely.**

`reachable: false` is **reported, not refused**, and that is the load-bearing choice. A dev server that is still compiling is indistinguishable from a port that will never answer, and opening the forward *before* the thing behind it is listening is the ordinary order of events. Tearing it down would break the common case to catch a typo. Looking again keeps the same local port, so a browser tab already open on it starts working rather than needing a new URL.

**Client-only, by type.** What comes back is `http://127.0.0.1:<port>` — an address on the machine that opened the tunnel. Handing that to a browser on a phone would be a URL pointing at the wrong computer, which is a subtler wrongness than a missing feature, so `previews` joins `screen`, `selectRegion`, `clips` and `speaker` in what §14's web path cannot be given. The refusal is asserted beside theirs, and writing that assertion found the handler checking the host *before* the client-only guard — so a browser got "no attached host" rather than the honest sentence.

**The host offers them.** `preview.ports` is a session-protocol command at v3 — asked of the host because the host is where the answer is, and the whole point of the feature is a port on a machine this one cannot see. The picker shows what it found beside the typed field rather than instead of it, since an empty answer is ordinary. A port already reachable from off that machine is marked, because forwarding one is harmless and knowing it is open is not.

The command reports `[]` where it cannot look, which is the asymmetry §12.1's capture path already draws: asking what is available is a question a UI asks on open, so an unanswerable one must not become an error banner. The *other* kind of "cannot tell" — a host too old to have the command — comes from `supports()` instead, and says which command and which version.

The control is shown for remote sessions only, because a local dev server is already on `localhost` and a button that does nothing visible teaches people the feature does nothing.

**Adding the command found the stranding bug the protocol negotiation exists to prevent.** `preview.ports` is the first command added since §17.16, so bumping to v3 was the first real exercise of it — and `connectRemote` refused *any* version difference, in either direction, before the handshake could speak. The check described itself as "the same rule the handshake enforces"; it was not. The handshake refuses a client older than the host's `MIN_CLIENT_PROTOCOL`, and the client declines commands the host is too old for. This refused the connection.

Reproduced against a real host rather than argued: a v2 host that had been running there since the previous day, with this app at v3, gave `REFUSED: the host on build-01 speaks session protocol v2, this app speaks v3: stop it there, or update this app`. "Stop it there" for a host holding a live agent means losing the work — exactly what §17.16 was written so that a bump would never require. Removed, and the same host attached on the next run at the same pid, reporting v2, with `preview.ports` unavailable and everything else working.

Two smaller things the same work turned up. `HostConnection.previewPorts` and `hasBlob` were typed `Promise<T>` and threw **synchronously** from the version gate, so `.catch()` on either would never have seen it — both are `async` now. And the host's own loopback control port is excluded from what it offers, read through a closure because the server is constructed before the listener binds: passing the number would have passed `undefined` forever.
...
**Which has a consequence found in the field.** A turn whose agent died leaves its session in `working` for good — correctly, since §10 refuses to let a suspicion move the state — and `shutdown` then refuses on its behalf while nothing in the CLI interrupts. Such a host cannot be asked to exit at all: upgrading one means killing the process. That is safe (§5.4 — the log is truth and every session reopens from it), and it is a gap rather than a decision. Restarting clears it, because a session that is merely on disk does not hold the host busy. **Now closed.** `agbrte interrupt` reaches it from a terminal, and an explicit interrupt resolves a session that is `working` with no handle registered. That inference is sound because the manager owns the turn loop: a handle exists before the state becomes `working` and is released when the turn ends, so `working` with an empty handle map means nothing is running and nothing else will ever move it. It lands in `awaiting_input` per §4.1 — nothing is broken, the work stopped and is waiting for whoever stopped it — and the reason is recorded, because a state change with no explanation reads as a bug six months later.

Only on an **explicit** interrupt. A timer that resolved sessions would be stall detection issuing the verdict §10 denies it.
...
**Exercised against a real screen and a real remote host.** `capture/electron.ts` was the one file nothing had ever executed — every capture test runs against a fake `ScreenBackend`, because `desktopCapturer` needs a compositor. The Electron smoke check now drives the real one, and the DPI arithmetic that was pure guesswork holds: a `1707×1067 @1.5` display yields a `2560×1600` grab, which is what asking in device pixels rather than DIPs is supposed to do. The full client path — take, draw, redact, store, transfer, flatten, describe — has also been run end to end against the tailnet server, where the described arrow tip landed within a pixel of where it was drawn after the preview→stored conversion.
...
**Two bugs in the headless path, found by its own tests and neither environmental.** `chrome.exe --version` on Windows prints nothing and never exits, so probing with it stalled ten seconds and then *skipped Chrome* — every capture paid the delay and used Edge instead, which is not a browser anybody chose. An absolute candidate is now checked by existence, which is the question being asked. And without `--user-data-dir` the browser used the default profile: with that browser already open it handed off to the running process, exited 0, and wrote no file, surfacing as `ENOENT` on a temp path that explains nothing. Both were silent, and the second only appeared once somebody had a browser open — which is most of the time.
...
- **So does an image that is not sent at all**, and that was a bug rather than a decision. An agent with `input.image: false` got the placeholder and *nothing about the arrow*, which inverts this section's own argument — the description "is often the only part a weaker vision model reads", so the agent that cannot see the picture is the one that needs it most, and it was the only one not getting it. The same held for an image dropped over a count limit or too large to resize. Found by running a capture against a real remote session, not by reading the code: the turn arrived as two text blocks with the annotations nowhere.
...
**The drawing surface is built, and §12.1's constraint decided its shape.** A blackout drawn after the first store cannot unwrite the original, so capture is **two steps with the drawing in between**: `capture.preview` grabs, crops, scales and holds the frame **in memory, unstored**; the user marks it; `capture.commit` splits the blackouts out, paints them, and only then writes. This is what makes "the annotator must offer redaction at capture" a mechanism rather than an instruction — there is no moment at which an unredacted frame is on disk, because the store happens after the decision.
...
- **Cancelling says so.** Closing the annotator discards the held frame rather than waiting for the TTL, because the difference is a screenshot of somebody's desktop living five more minutes or none.
...
*Status, checked criterion by criterion.* "Notified exactly once per completed session" is covered squarely — `notify.test.ts` asserts the announcement, that it does not repeat while the state holds, that it fires again when the thing genuinely happens again, and that twelve children finishing is one notification rather than twelve. "A quota-exhausted agent parks and resumes on its own at reset" likewise: it parks with a reset time, picks the *same turn* back up, and stays put until the window actually turns.

"Ten concurrent sessions" had never been run. It has now, and it works: ten turns in flight at once across three workspaces all complete, each session's transcript contains its own marker and none of its neighbours', and usage is accounted per agent rather than pooled. That last assertion is the reason to run it — ten turns interleaving through one manager is where a misfiled event would surface, and it would look like a model saying something strange rather than like a routing bug.

Two things that criterion asks for are still unverified. It says *three models and two auth modes*, and the test uses `echo` throughout — the variety is a claim about admission and capability resolution, not about concurrency. And the per-host concurrency cap did not exist — ten sessions ran because nothing was limiting them rather than because queueing worked. **It is built now**, and the order it sits in is the decision: a slot is taken *after* the credential wait, because waiting on a shared allowance costs this host nothing and holding one of its scarce slots through that wait would starve an agent on a different credential. §8's "three independent limits" is what that separation buys.

A slot is held for as long as a turn runs, **including while it is blocked on a permission prompt** — deliberate, because the worker is resident and costing memory whether or not a human has answered, and this cap bounds memory rather than activity. A parent waiting on children holds nothing, since `propose_split` returns inside the turn; without that a tree deeper than the cap would deadlock.
...
*Status — the "different machine" half is not met, and the table said it was.* `spawnChild` creates the child through the parent's own `SessionManager`, which owns one workspace on one host. A `target` naming another machine set the child's `target` field and changed nothing about where it ran: the log said `ssh` while the agent worked locally, and `ChildRef.instanceId` was the parent's. That is worse than the feature being absent, because an absent feature is noticed and this one was only noticed by whoever later trusted the field — roll-up asking a machine that never had the session, or a person reading a transcript.

It refuses now, naming the gap rather than the caller. §4.3 already recorded the two consequences of hosts being plural as open — tree budget having no single owner, and `needsAttention` bubbling across hosts — and building the spawn needs those answered first, because the fleet rather than a manager would have to own it. §15's table now agrees with §4.3 instead of contradicting it.
...
**WSL/container/k8s transports are not built, and the attempt to build one is what produced the piece that is.** The plan was WSL, on the reasoning that this machine could actually run it; the machine has no WSL, so writing it would have meant shipping a transport nobody could cross — the exact failure this table already records twice. Looking into it instead surfaced two things that could be verified here. The connector's `if (kind === 'ssh')` sent the other six kinds to the *local* branch, so an unbuilt locality ran on the user's own machine rather than failing (§16). And the reason those four are not a runner swap is structural, not a matter of effort: the host is reached through a **unix socket**, none of them can carry one, and §6.2's stated fallback — a loopback control port plus a bearer token — has never been implemented. That is now the recorded blocker for all four, instead of something the next attempt rediscovers.
...
| **An execution target that quietly runs somewhere else** | six of `ExecutionTarget`'s eight kinds fell through the app connector's `if (kind === 'ssh')` to the *local* branch, so asking for a container started a host on the user's own machine — a real session, a real transcript, and a badge saying `docker:…`. Unreachable from the UI, which builds only the two working kinds, so it never fired | **Closed.** A `Record<TargetKind, TransportDescriptor>` makes a new kind a compile error until it is described, and `Fleet.attach` refuses an unimplemented one by name *above* the injected connector — the placement matters, since the check the app had would not have been the CLI's. The test asserts the connector is **never called**, because the error is the easy half and the silent local run was the failure. Verified by removing the gate: `expected "vi.fn()" to not be called at all, but actually been called 6 times` |
| **A version bump that strands the hosts it upgrades** | §17.16 built negotiation so an additive bump would not require killing running hosts — and `connectRemote` refused any version difference before the handshake could speak, telling the user to "stop it there". For a host holding a live agent that is lost work, and it would have fired on the first bump after negotiation shipped | **Closed.** The pre-flight refusal is gone; the handshake decides, which is where both halves of the rule already live. Reproduced against a real v2 host running remotely with this app at v3, then re-run: same pid, attached, `preview.ports` unavailable and named, everything else working. The lesson is that a guarantee tested only by the layer that implements it is untested — negotiation had tests, and the layer above it had the bug |
| **One question stopping a whole machine** | a turn blocked on a permission prompt held its concurrency slot. `defaultTurnCap()` is `min(8, cores − 2)`, which is **1** on any machine with three cores or fewer — a small VM, a CI runner, a Raspberry Pi — so one unanswered question stopped every agent on the host, on every session, until a human came back. Deliberate, and documented as such: "the worker is resident and costing memory whether or not anyone has answered" | **Closed.** The slot is handed back while a person is waited for and retaken before the tool runs, which is the call the design already makes one line above the acquisition, where a quota wait sits outside the slot because "waiting on a shared allowance costs this host nothing". Waiting on a person costs it nothing either, and a prompt can go unanswered for hours — that is §11's whole premise. Every development machine here has four cores or more, so it passed everywhere it had ever run; **found by CI on a small runner**, surfacing as an unrelated tree test timing out on all three platforms at once. The regression test pins the cap to 1 rather than leaving it to the hardware, which is what hid it |
| **A socket outliving the host that made it** | a unix socket is unlinked only on a clean close, so a killed host leaves the path behind — after which every future host fails to bind and the workspace can never be opened again. The user is told "host did not start listening", fifteen seconds later, naming the host rather than the leftover. `discovery.ts` had solved exactly this for `host.json` and called it "the classic stale-pidfile deadlock"; the socket is a file too and was not given the same treatment | **Closed.** A bind conflict asks whether anything is actually there: nothing answering means debris and is removed, something answering means a real owner and is refused by name (§6.6 single writer, §17 Q9). Measured on a real Linux host — `SIGKILL`, socket remains, `EADDRINUSE` on bind, `ECONNREFUSED` on connect — because Windows named pipes leave no filesystem entry, so this was a bug that existed only on the machines remote execution is for. Residual: another user's stale socket under `/tmp`'s sticky bit cannot be removed by us, and that path says so rather than looping |
| **A persistent process started on a model's say-so** | §3.12's reaping — whatever an agent starts, ends — is real containment. An API that starts a process outliving the turn launders it if a model can reach it: "run this in the background" becomes possible again by asking us | **Closed, two ways.** `preview.start` is a protocol command and an IPC method and is absent from the tool registry, so a model cannot name it; and it is a write, so §7's read-only client is refused — asserted, including that nothing was spawned. Residual: it runs an arbitrary shell command as the host user, which is the same authority the user already has there and is theirs to exercise, but it is not sandboxed and is not claimed to be |
| **A field that is declared and never written** | The failure this project produces most reliably, and `template.target` was the third instance in one session: a type states a fact, no code ever supplies it, and a reader gets `undefined` and concludes the opposite of the truth. Compilers do not catch it — an optional field with no writer is valid TypeScript — and tests do not either, because a test that constructs the value by hand supplies exactly the state reality withholds | **Swept, not waited for.** Every field of the promise-bearing shared types was checked for a reader and a writer. Two had neither anywhere in the repository, including tests: `PermissionRequest.originSessionId`, which §7's table asserted the host stamps, and `ChecklistItem.delegatedTo`. Both described parent↔child links that nothing could build, and both were removed rather than implemented — the mechanism `originSessionId` reached for already exists as `needsAttention.from`, and a second never-populated route to one fact is how two mechanisms drift until one is quietly wrong. Both deletions compiled with zero errors, which is the proof they were dead. Four `RuntimeCapabilities` fields (`streamingToolArgs`, `toolResultPairing`, `reasoningVisible`, `tokenCounter`) are written by every adapter and read by none; they are declarations that travel to clients inside `resolvedCapabilities`, so they are unconsumed rather than dead — a weaker state, recorded here so the difference stays deliberate. **The mirror image is a name referenced and never defined**, and the renderer had four: `btn-quiet` on ten buttons across six files, plus `text-state-failed` (a typo for `state-fail`), `hover:text-fg` and `text-warn`/`text-ok` for tokens that do not exist. Each renders as inherited body text, so nothing looks missing — the words are simply the wrong colour, and the words were error messages. `scripts/inert-classes.mjs` compares the renderer's class names against the *built* stylesheet, because a class is real exactly when it survives into the output, which is also what the browser sees. CI runs it `--strict` after the build; it exits non-zero, since a checker that only prints is a checker that passes. **And the largest version of the same shape is a whole module nothing reaches**: `windowsBootstrap.ts` passed an end-to-end test over real ssh while `connectRemoteHost` still ran the POSIX path against every remote, and the only reference to it outside itself was a comment claiming the feature worked. `scripts/unreached.mjs` walks imports from the build's own entry points — read out of `scripts/build.mjs` and `package.json` rather than guessed, since a missing root makes everything under it look dead — and follows dynamic `import()` too, which was its second false positive. It separates *reached only by tests* from *reached by nothing*, because the first is the dangerous one: built, covered, green, and wired to nothing. Verified by unwiring `windowsBootstrap` again and watching it be named |
| **A stop that leaves the port bound** | `npm run dev` is npm, which spawns the node that listens. Killing the child leaves the grandchild holding the port, and the next start fails with `EADDRINUSE` for a server the user believes they stopped | **Closed.** Process group on POSIX, `taskkill /T` on Windows — where `detached` creates a console rather than a group and the first implementation was silently wrong. Caught by watching a grandchild keep writing a file after its parent was killed, which asks the question from outside the process tree |
| **A preview that opens onto nothing** | `ssh -L` binds the local end whether or not the remote port exists, so a forward to a port with nothing behind it reports success and fails later as a reset — which reads as "the preview is broken" and sends the user to debug the wrong machine | **Closed.** Each forward is probed; measured at 4–5 ms for a dead one against indefinitely for a live one. Reported rather than refused, because a dev server still compiling looks identical to a dead port and opening the forward first is the ordinary order — rechecking keeps the same local port so an open browser tab starts working |
| **A port list that is reconnaissance** | `/proc/net/tcp` shows every listener on the machine with a uid column; offering the lot tells you what your colleagues are running, on which ports, right now. The capture used to verify the parser had twelve listeners under four uids, one of them another user's service on `0.0.0.0:4000` | **Closed.** Narrowed to the uid the host runs as, which is also the only useful answer — somebody else's port is not a preview of your agent's work. The bind address is carried through rather than flattened, since `127.0.0.1` and `0.0.0.0` differ in exactly who can reach them |
| **A capability table nothing reads** | §6.2 said capabilities are "enforced, not assumed" while `TransportCapabilities` was declared in a type file and referenced by no code at all — the same shape as `spec.limits` before it | **Partly closed.** They are read by the refusal, which names the capability blocking each unbuilt locality; that is what turned "WSL isn't written yet" into "four transports are blocked on one missing control channel" — and the channel was then built, so the rows now say what is actually left. The runtime gates §6.2 describes — `persistentProcesses: false` disabling detached runs, `portForwardIn: false` forcing a target-local endpoint — still have no implemented transport that declares `false`, so they remain unexercised and marked `documented`, not `observed` |
| **A control port is not a unix socket, and the host treats them alike** | `grantRole` gives a client the role it asks for because "reaching the socket already proved who you are" — true of `0600`, false of a loopback port every process on the machine can dial. Serving the session protocol over TCP without replacing that proof would hand any local process, including a browser, a `read-write` client | **Closed for the channel, open for the transports that will use it.** The token is stored in the host record under `.devagents/`'s `0700` and written `0600`, so reading it needs the permission that reaching the socket needed; it is verified *below* the protocol, so an unauthenticated connection cannot issue `session.list`; the listener binds `127.0.0.1` explicitly, asserted by dialling a non-loopback address of this machine and requiring refusal. Residual: the token is at rest in a file, where a unix socket's proof was ambient — a workspace on a filesystem that cannot express `0600` (a mounted share, an exFAT stick) would leak it, and nothing detects that yet |
...
| **Client capability assumed uniform** | a capture or dictation button offered on a device with no desktop to capture, failing at the point of use instead of being unavailable | **The named failure is closed; the general mechanism is not.** Capture and dictation are now *asked* rather than assumed: `capture.sources()` returns empty on a client with no screen so a picker renders "nothing to capture", `voice.status()` reports unavailable with what to install, and the composer shows "no mic" instead of a button that fails when pressed. The web server is additionally forbidden by type from holding a screen, an overlay or a clip store, because it is reached over a network and the failure there is a browser capturing the *server's* desktop. Still open: a capability *descriptor* per attached device, rather than one question per capability |
...
| **A tool result that cannot carry an image** | `ProviderMessage`'s tool role is `result: string`, so a screenshot cannot be returned as a tool result. Any screenshot-in / action-out loop — computer use, visual iteration on the agent's own output — is unrepresentable, not merely unimplemented | **Closed.** `ToolResult.blocks` carries content, and the harness delivers it as a **user** message straight after the tool result rather than inside it: providers reliably accept images in a user turn and many reject them in a tool role, and an image delivered where the provider will not look costs a request to find out. Done before capture was built on the old assumption, which is what the row asked for |
...
| **A guarantee held by nothing** | the strongest sentences in this document are the ones nothing enforces yet, and they read as true right up until the feature they protect arrives. §12.1's redaction held *by failing* — no painter existed, so nothing could be stored. §12.4's "audio never reaches a model provider" had no branch in the fitter at all; it was true because nothing produced an `AudioBlock`. Both would have stopped holding on the day the feature landed, silently | Write the enforcement with the promise, not with the feature. A guarantee needs a test that would fail if the guarantee were removed — `fitContent` converting audio *even for an agent that accepts audio* is that test, and it is the one that distinguishes a guarantee from a downgrade |
| **A component that passes its own tests and is reached by nothing** | §12.3's vector model, describer and flattener all existed, were all tested, and **nothing outside `content/` called any of them** — so the phase table read "built" while a user could not point at anything. The same shape produced every §12 bug this session: the annotation description dropped on the one path that most needed it, and a capture test that rebuilt the seam by hand and lost the marks | §15 now says a component counts only when something a person touches reaches it. The check that finds these is not a unit test — it is running the sentence end to end, which is what Phase 7's acceptance criterion is for |
| **An operation whose name says read and whose body writes** | `blob.has` answers a question *and*, on a miss, copies the blob from a sibling session — so a read-only client could make the host write files. The name is what made it invisible: it was reviewed as a query and gated as one | Gate on what the body does, not what the verb suggests. Found by auditing §13 against the code rather than by a failing test, which is now the third time that has been true of that section |
...
| Screenshot leaks secrets across a network | credential exposure off-machine | **Mostly closed, structurally rather than by discipline.** Capture is explicit only; the frame is held **in memory, unstored**, while blackouts are decided, so there is no moment at which an unredacted frame is on disk to be pushed; painting happens on the machine that took the picture, which §6.7 made necessary once a capture could cross a network; cropping is applied to the bytes rather than as a viewport, since what lies outside the rectangle is what the user chose not to send; and every painted rectangle is recorded in provenance. The OCR pre-pass is **not built** — it is a native model and stays injected — so the sweep reports `scanned: false` rather than an empty match list, because "looked and found nothing" and "could not look" are different facts |
...
| Electron capture platform gaps | silently black frames | Pre-flight `getMediaAccessStatus` **before** the grab, not after: on macOS a denied capture does not fail, it returns an empty desktop that is indistinguishable from a tidy one, so checking afterwards turns an error into a mystery. `unknown` is its own status and proceeds, because Windows has nothing to report and silence is not a refusal. Verified against a real display through the Electron smoke check, including that the grab is a display rather than a thumbnail. The Wayland portal path is **not built** |
| Process overhead at scale | memory pressure | **Closed.** Quota-group throttles, idle parking of hosts, and now the per-host cap — `min(8, cores − 2)`, FIFO above it, taken after the credential wait so a local slot is never held waiting on a remote allowance. The queue is strict: a turn arriving as a slot frees waits behind whoever asked first, because on a busy host the alternative is one session that never moves |

---

## 17. Open questions

1. **How much harness should `AgbrteHarness` be? — answered by where things kept landing.** It stayed thin, and not by discipline: every capability that looked like it belonged in the harness turned out to belong in the session host, because the host owns the blob store and the log and the harness owns neither. Image fitting was written into the harness and moved out when its resizer could not reach the bytes; capture returns a stored `ImageBlock` rather than pixels for the same reason; the message bus and split proposals reach the owner through `RuntimeContext` instead of acting locally. The rule that emerged is sharper than "start thin": **the harness may decide, and may not own.** Subagents and progressive instruction loading remain unbuilt, and would be the first things to test it.
2. **Automatic model routing — recommended: no, and the reason is stronger than "wait for usage data".** With cost and capability data the app *could* pick a model per role, or per turn (cheap for mechanical edits, escalate on failure). The original objection was reproducibility and unpredictable cost. The sharper one is that **the input does not exist for the runtimes this project prioritises.** §10 has three cost fidelities and `'unknown'` is contagious: a local model is free, and an installed CLI under `vendor-cli-session` reports `costReporting: 'none'` because the spend is on the vendor's own meter. So routing-on-cost would route on a number that is absent exactly where the design points people — their own machine, their own CLI.

    Escalate-on-failure has the same shape. "It did badly" is not observable; `StopReason` gives `error` and `refusal`, which are already one glance away in the transcript and are the cases a person handles in a second. Automating the visible half while the invisible half stays invisible buys little and costs reproducibility.

    **What is worth building instead is suggestion from capability, which is deterministic.** §12.2 already knows an agent declares `input.image: false` and downgrades a screenshot to a description; saying "a runtime on this host would see the picture" is a fact, not a guess. That is the honest form of the idea, and it needs no cost data at all. What would change the answer for cost: a `costReporting: 'exact'` gateway across a whole fleet, which is a §6.5 feature and not a routing one.
3. **Should quota state be shared across machines?** A `quotaGroup` for a subscription is really global to the user, but Agbrte only sees what it schedules — usage from the vendor's own app or another device is invisible. Optimistic scheduling plus reactive parking is the pragmatic answer; a vendor usage API would be better where one exists. **Built as leaned** (§8), and it added a second blind spot of the same shape: the scheduler lives in the session host, so a credential group spanning two hosts is scheduled by each separately. Both are the same question — what we cannot see, we cannot pace — and both are answered by reacting rather than predicting.
...
5. **Should a child ever run in a different workspace than its parent's repo? — built, and the answer came from splitting the spawn rather than from owning anything.** The schema allows it and the cross-repo case needs it, but a tree spanning four `.devagents/` directories is hard to reason about and any one of them can be unreachable. Possibly cap cross-workspace children at depth 1.

    *Sharpened by trying it.* This was written as a design question and is also a build one: spawning is single-host, so the answer today is "it cannot", and the field that said otherwise was mislabelling sessions. Whatever is decided, the mechanism has to move to the fleet — a `SessionManager` knows only its own workspace, which is the same reason §4.3 says tree budget has no owner once hosts are plural.

    *Halved by reading it.* §4.3 names two things blocking this — tree budget having no owner, and `needsAttention` bubbling across hosts — and **the first one dissolves on inspection.** Budget is not a shared ledger: `reserveForChild` debits the parent at *spawn*, before the child exists, and hands the child a ceiling embedded in its brief. Nothing is released back — there is no refund path anywhere — so the parent's number is only ever touched on the parent's host, and the child enforces a figure it was given. "A tree cannot outspend its root" is maintained by that debit, not by anyone owning a total. There is nothing to own, so plural hosts change nothing about it.

    What remains is one thing, and it is **not** the bubbling either. `rollUp` walking a single host's map is a real limit, but the fleet already receives every session from every host and already re-sorts them globally by attention, so raising a descendant's attention onto an ancestor is a derivation over data in hand — no new command, no cross-host write, and nothing recorded in one machine's durable log about another machine's. It is deliberately *not* built yet, because cross-host parent and child cannot exist while spawn refuses them: code prepared for a case that cannot occur is the failure this project keeps finding, and writing it early would only make it stale.

    So Q5 reduces to **fleet-owned spawn**, and the shape follows from where the pieces already are: reserve and build the brief on the parent's host, create the session on the target host, then record `session.spawned_child` back on the parent's. Two of those exist; the missing piece is a way to ask a host to reserve and brief *without* creating, which is the one genuine protocol addition in the whole question.

    *Built.* `prepareChild` decides and changes nothing, `createSession` takes the position and brief a child inherits, `recordChild` commits — and the fleet runs the three against two hosts. No two-phase commit, for the reason above: the debit lands after the child exists, so a creation that fails on the far host leaves nothing behind on the near one.

    Two things the wiring forced into the open. `session.create` carried a title and a goal, so every other field a session can be made with — its budget, its policy — was dropped at the wire and silently defaulted on the far side; invisible while only a local manager made children. And **the approver picks the machine, not the proposing agent**: §4.3 keeps splits user-approved because a decomposition mistake is expensive, and the person approving is the one who can see the fleet. An unattached host is refused by name rather than quietly run locally, which is the original bug in a new hat.

    The bubbling followed, once a cross-host child could exist to bubble from. It is a derivation over sessions the fleet already holds — no new command, and nothing written into one machine's durable history about another's, because a view can be recomputed on the next list and a log entry cannot be taken back. The descendant's own reason travels up with the same `from` breadcrumb `recomputeAttention` attaches; two bubbling rules that disagreed about what a raised attention says would be worse than one that stopped at a host boundary, because the disagreement would be invisible. §15's criterion — *a permission prompt raised by the deepest child appears in the top-level Needs-you rail* — now holds across machines as well as within one.
6. **Can a child spawn its own children, and should the parent know? — trees exist, and the answer split in two.** `maxDepth` 3 permits it. The tension was that full visibility scales badly while direct-only hides stalls, and building it resolved that by separating the two things travelling upward: **structure is direct-only** — a parent holds `ChildRef`s for its own children and nothing deeper — while **blockage is total**, bubbling from any depth to the root with the breadcrumb attached. So a grandparent does not carry its grandchildren, and still cannot miss one waiting on a permission prompt. Neither original leaning was right; the question was really two questions wearing one sentence.
7. **Result contract enforcement — answered: no, and the question conflated two actors.** `summaryMaxTokens` is enforced, verified: an oversized summary becomes an artifact plus a pointer rather than a refusal, so work described at length is not thrown away for the length while the child still gets no larger injection.

    The worry was that a parent could read the 40-page artifact back and reintroduce the explosion by hand. It cannot, and the reason is not in the contract — **every tool truncates its output** at 8,000 characters, `read` takes no offset so a loop cannot page around it, and reaching past that means `bash`, which §13 leaves at `ask`. A parent that spends turns reading is then bounded by its own `tokenCeiling`, which is enforced as of this session.

    And "a parent choosing to read it" is two different actors. A **person** opening an artifact is not context entering a model and needs no budget at all; a **parent agent** calling a tool is, and is already capped four ways. Budgeting artifact reads specifically would be a fifth mechanism for a path three already close — which is the over-policing the question asked about.

    The answer rests on a property nothing was guarding, so `tools.test.ts` now asserts it: adding an `offset` to `read`, or a tool that returns a whole file, would make this false silently.
8. **Automatic splitting — built: approval stays, and the grant is a budget rather than a mode.** The signals in §4.3 are measurable enough to act on, and always requiring approval means a long overnight run stalls at the moment it most needs to decompose.

    But the stated motivation is a *stall*, not a split: the problem is that approval blocks at 3am, and that is a §11 parking-and-inbox problem wearing a §4.3 hat. The fix that follows is **pre-authorisation at session creation** — "this run may split up to N times within this budget and depth" — decided when the person is present and thinking about the run, rather than inferred at the moment they are not.

    Three properties make that safe, and they are what turns "a mode" into "a budget". It is **spent**: two of three used is visible, and the third is the last. Every auto-split still writes the same `session.split_proposed` event and still reaches the inbox, so it is reviewable afterwards rather than invisible — §4.3's reason for approval is that "a decomposition mistake made at 3am is expensive", and the expense is in not being able to *see* it, not in it happening. And the grant is per session, so it cannot leak into the next one by being a setting.

    **The grant is spent after the child exists, not before.** `respondSplit` can still refuse — a limit, a malformed brief — and decrementing first would charge a run an allowance for a split it never got a child out of. Found by a test fixture whose `outOfScope` was empty, which §4.3 refuses on purpose.

    A child does not inherit it: being allowed to decompose is not the same as being allowed to hand that permission down, and a grant that propagated would be one decision authorising an unbounded tree. And a grant of zero is stored as no grant at all, because a count nobody can spend is a field that only exists to be misread.

    The transcript is the part worth checking, and it is asserted: an automatic split writes `session.split_proposed` **and then** `session.split_decided`, in that order, exactly as a manual one does — so a decomposition made at 3am can be read in the morning. The decision is attributed to `agbrte:split-grant` with `via: 'asserted'` rather than to the person who created the session, because §5.1's rule is that an actor records what established it, and a transcript that credited the user could not answer "did I approve this one".
9. **Shared remote hosts, multiple users — answered: share the box, never the workspace, and the rule was already there.** One host per workspace per user today. A team sharing a build box and *checkout* was framed as needing per-user workspaces or a multi-tenant host with per-user log scoping.

    It needs neither, because §6.6 already forbids the case: **two users driving agents in one checkout are two writers**, and single-writer + append-only is what buys "no merge, no conflict resolution and no vector clocks anywhere in this design". Multi-tenancy on one workspace is not a missing feature; it is a thing this design cannot have without giving up the property the whole persistence story rests on. Per-user checkouts on a shared box are supported now and are the answer.

    What was missing was enforcement, and looking for it found a bug that had nothing to do with users. The socket path is keyed by `instanceId`, so a second host on one checkout collides — and the collision was **indistinguishable from a socket left behind by a host that was killed**. `discovery.ts` had made `host.json` a hint for exactly this reason ("the classic stale-pidfile deadlock") and the socket had not been given the same treatment. Measured on a real Linux host: `SIGKILL`, socket file remains, next `listen` gives `EADDRINUSE`, connecting to it gives `ECONNREFUSED`. **After an unclean death the workspace could never be opened again**, and the user was told "host did not start listening" fifteen seconds later. Invisible here because Windows named pipes leave no filesystem entry — a bug that existed only on the machines remote execution is for.

    A bind conflict now asks the question the record already knew to ask: is anything actually there? Nothing answering means debris, removed. Something answering means a real owner, refused by name — which is where a colleague's host on a shared box gets a sentence instead of a timeout. The remaining gap is honest and small: another user's stale socket under `/tmp`'s sticky bit cannot be removed by us, and that path says so rather than looping.
10. **Non-deterministic CLI mode — and it is no longer only a policy choice.** Building `agent-cli-stdio` (§3.12) turned up a constraint that settles half of it: Claude Code's deterministic flag skips OAuth and keychain reads, so under `vendor-cli-session` auth the user's own login *is* the thing deterministic mode declines to read. The two cannot both be honoured, and the adapter derives the mode from `AuthMode` rather than offering it as a switch that could be set to contradict itself. What remains open is only the SDK branch. Narrowed by Phase 1: the mode does not exist there. The SDK adapter pins `settingSources: []` unconditionally and `buildOptions` throws if a later edit reintroduces a bypass, so "use my local setup" is currently disallowed by construction rather than by policy. What remains open is whether to add it at all — inheriting the user's hooks, skills, and MCP servers is genuinely useful, and the cost is a transcript we cannot promise to replay. If it is added, the honest shape is a per-session flag recorded in the log, so a non-reproducible session is labelled as one rather than discovered later.
11. **`RuntimeDescriptor.requiresModel` was a boolean answering a three-valued question — answered: it is now `model: 'required' | 'optional' | 'none'`.** Required for `AgbrteHarness`, which has no loop without a provider; **none** for `echo`, where a model would be silently ignored and admission should say so; **optional** for an installed CLI, which authenticates itself, ships a tuned default, and also takes `-m`.

    The two answers a boolean could give were the wrong two. "Optional" had to be spelled `false`, which admission read as *not applicable* and rejected — so naming a model for an installed CLI was unreachable, and `modelArgs` was left out of the manifest rather than shipped as code admission guarantees never runs. Both now exist: `--model` reaches the argv when the user names one, and the CLI's own default stands when they do not, because the vendor's choice for their own harness is a real answer rather than an absence. The renderer shows the field for anything except `none`, where before it showed it only for `required`.
12. **Session templates — built, derived from sessions rather than authored.** Templates would carry rosters, checklists, a default target, and a model + auth assignment per role.

    Every one of those fields is already in a `Session` record. So the feature is **"save this as a template"** — a projection of a session that worked — rather than a document format someone fills in ahead of time. That difference is most of the value: a template format is a thing people configure once and then drift away from, while a captured session is a thing they already have working in front of them at the moment they notice they will want it again. It also inherits provenance for free, since §5.1 already records which model and auth each agent actually resolved, so a template says what *ran* rather than what was intended.

    The one field that must not be captured is any credential, and §13's rule already covers it: the template records the auth *mode* and the endpoint id, never a secret. That is asserted rather than assumed, because a future field on `AuthMode` would arrive in a committed file for free.

    **Committed, and that is the point.** `.devagents/`'s own `.gitignore` excludes `sessions/`, `index/`, `run/` and `instance.json` — derived state — and tracks `project.json` and `memory/`. Templates join the tracked side: "this is how we run a review session in this repo" is a fact about the project, and the kind of thing a new colleague should get by cloning rather than by being told.

    **The `target` field was recorded, not enforced — twice over.** Its own documentation said what had to happen ("a user who does not have that alias gets a refusal naming it, which is more useful than silently running somewhere else — the failure §16 already records for execution targets") and `template.apply` never read it: it rebuilt the roster, the goal and the checklist and started the session wherever it was asked. Nothing looked broken, because the field was *also* impossible to populate — the projection took it from `session.target`, which is `{kind:'local'}` on every session that has ever existed, since `session.create` carries only a title and a goal and `spawnChild` refuses a target differing from its parent's. So a promise sat in a doc comment being believed, with no reachable code either to keep it or to break it. The test that covered it passed throughout, because it built a session with `target: ssh` by hand: **a fixture that constructs an impossible input tests the function and not the feature.**

    The correction is where the value comes from. A target is not a property of a host — a host on the build box knows a workspace, not that it is "the build box" — it is the *client's* name for the route, held by `Fleet`. So `template.save` gained the field, the fleet stamps its own entry's target on the way out, and the fleet refuses on the way in, naming the machine. It refuses rather than attaching the right host on the user's behalf: the alias may not exist here, may point somewhere else, or may be a machine they did not mean to start work on, and naming it is the useful act.

    This is also the first version bump that adds a *field* rather than a command, which `COMMAND_SINCE` cannot express — `supports('template.save')` is still true against a v5 host, which ignores the field and writes a template with no target that applies anywhere, exactly as before. Safe because the missing field drops a restriction that never existed, rather than changing a result; a field where that is not true needs `MIN_CLIENT_PROTOCOL`, which is the lever that exists for shape changes.

    **Dropped on purpose:** ids, state, usage, resume tokens, artifacts, and `resolvedCapabilities` — a snapshot of the host that run happened on, which would be a lie on any other. Checklist items keep their text and lose their progress, because a template of a finished checklist is a checklist that starts finished. A local target is dropped too: it is the default everywhere and recording it would make every template claim a locality it does not care about.

    **The name becomes a filename**, from a string a person typed, so it is built from an allow-list rather than by removing the dangerous parts — `..`, a separator, a drive letter, a NUL and a leading dash are each their own escape, and a deny-list is a promise to have thought of all of them. An id arriving over IPC goes through the same function again, because an id someone else chose is not ours for having been ours once.

    **`apply` is a host operation**, not a client loop of create-then-add. The roster that runs is the one in the file the host read; a client assembling it from a template it fetched would be a client that can quietly assemble a different one. Seats are admitted one at a time rather than in parallel, so a roster that half-applies stops at the seat §3.10 refused instead of racing three more past it.

    Commands `template.save` / `list` / `apply` / `delete` at protocol **v5**. Saving and applying are writes — one puts a file in the repo colleagues will pull, the other spends money on somebody's behalf — so §7's read-only client is refused both and keeps the list.
13. **Memory curation — recommended: propose-only for deletion, but let an agent supersede.** The question was whether agents may prune project memory autonomously; the leaning was propose-only, because silent memory loss is very hard to debug.

    That is right and incomplete. §5 says memory writes "surface in the UI so memory never grows silently"; the symmetric rule is that it must never *shrink* silently, which is the leaning. But propose-only alone means memory can only grow, and a file of confidently stated stale facts is its own failure — the recall path warns that a memory "reflects what was true when written", which is a warning about a problem nobody is allowed to fix.

    So: an agent may write a fact that **supersedes** an earlier one, and the earlier one is marked superseded rather than removed. Nothing is lost, the wrong answer stops being served, and the correction is a visible write of the kind §5 already surfaces. Deletion stays a human act, because deletion is the only operation that destroys evidence.

    Unbuilt: the `remember` tool does not exist yet, so this shapes a feature rather than describing one.
14. **What shape is the client? — answered: both, and the browser pays exactly the three costs predicted.** One renderer serves the Electron window and a WebSocket-backed browser client, so "any device" means any device with a network route rather than any device with an install. The three §14 costs came due precisely as listed and are recorded where each bites: client capture is unbuilt (§12.1), notifications cannot fire in a browser and `notify.ts` says so rather than silently no-opping (§11), and credentials stay host-side, which the endpoints file already assumed. The original framing below is kept because the reasoning is what made the answer cheap.

    **What shape is the client?** "Use it from any device" has two readings and they are different products. A desktop app per device, using SSH as its transport to the server, is what §14 assumes and needs no rethink — but "any device" then means any device you have installed on. A browser UI served from the server reaches anything with a network connection, and costs three §14 choices outright: `desktopCapturer` for client capture (R6), `safeStorage` for credentials, and native notifications (§11). X11-forwarding the Electron app is the third option and is rejected — painful off-LAN and it breaks capture.
15. **Two clients sending at once — resolved: a queue.** Of the three options considered (an exclusive owner, a queue, a soft lock), the queue is the one that does not require deciding who is "primary" — which a person switching between their own desk and their own phone should never have to answer. Turns are ordered by **arrival at the owner**, because with two clients that cannot see each other, arrival is the only ordering that exists.
...
16. **The protocol version check is all-or-nothing — answered: compatibility is a range, and each side owns one end.** The rule was equality, which is right when a field moves and much too strong when a command is added: §6.7 added two and stranded every running host. The field failure was worse than the tidiness one — `agbrte stop` speaks the *new* protocol, so the polite shutdown could not reach the old host it existed to retire, and a bump cost a `kill`.

    Now the **host** decides whether it will serve a client, refusing below `MIN_CLIENT_PROTOCOL` — the only case a version check must catch, since a changed command *shape* is the one thing a client cannot detect for itself. The **client** decides whether a command is available, from the `protocol` the host already reports and a `COMMAND_SINCE` table, so an older host costs one feature rather than the connection and says which at the point of use.

    The part that makes it deployable rather than a plan: **it needs nothing from the old host.** A v1 host ignores the new `hello` field and reports `protocol: 1` exactly as it always did. Verified against a real one — `agbrteHost.js` built from `c1448e0` — which connects, runs an ordinary session, declines `blob.put` by name, and accepts a polite shutdown.
17. **Migrating a live session between targets — recommended: require a clean tree and let git do it.** The context half is no longer speculative: `rehydrate()` is implemented and runs on every turn for any runtime without native resume, so the mechanism is exercised continuously rather than only on the migration path. What stays open is workspace *content* — whether the app syncs it or requires a clean git state.

    The latter, and the reason is now the same one that answered Q9 above: syncing content would make Agbrte responsible for a merge, and §6.6's single-writer invariant is the one place this design has no conflict resolution at all. Two questions reaching for the same missing machinery is a good sign the machinery is deliberately absent rather than merely unbuilt.

    The practical shape is a **precondition, not a feature**: a migration refuses on a dirty tree and says what is dirty, in the same way §9 refuses an `all-or-nothing` runtime under `shared` isolation. A refusal a person can satisfy in one command is a better answer than a sync they have to trust.

18. **Who compacts, when a runtime cannot reach the log? — answered: the runtime decides and the owner performs.** The design says the harness compacts at a high-water mark by calling the **same `rehydrate()`** used for moved workspaces, so the durable path is exercised constantly. Nothing is built, and the reason is a seam rather than an oversight: `rehydrate()` needs a `SessionStore`, and `RuntimeContext` — everything a runtime is handed — has no route to the log. That is deliberate. §17.1's rule is *the harness may decide, and may not own*, and the log is the thing it most conspicuously does not own.

    Two shapes fit that rule. A **compaction callback on `RuntimeContext`**, alongside `requestPermission` and `sendMessage`: the harness decides *when* — it is the only party holding the message array and the context window — and the host does it, exactly as the harness decides a tool call and the host gates it. Or the **host drives it**, watching token usage it already records in `usage` events and compacting between turns; simpler, and blind to the case that matters most, which is a turn that overflows partway through its own tool loop.

    The callback won, on the rule that produced every other answer here. `RuntimeContext.compact(budgetTokens)` is optional for the same reason `sendMessage` is — an adapter running its own loop has no message array to replace — and the budget travels *with* the ask so there is one opinion about it rather than two that can drift.

    What building it added to the question: the restriction to turn boundaries, which is not a simplification but a correctness requirement (tool calls and their results must stay paired, and a summary is neither), and the second threshold, without which a session sits at the mark and re-summarizes forever. Both are the kind of thing the host-driven shape would have had to invent as well.

19. **Answering every prompt is the point until it is the obstacle — decided: a per-session grant, never a setting.** A long unattended run stops at the first `bash`, which is §13 working and also the reason people reach for a global "don't ask me". The shape is the one §17.8 arrived at for auto-splitting, and for the same reasons: it is **granted per session**, so it cannot become a preference somebody turned on months ago and forgot; the grant itself is an **event in the log**, so a transcript says when the gate was relaxed and by whom; and every call it settles still writes `permission.decided`, so the audit trail is complete rather than empty.

    The `via` value is new and must be. Recording an ungated call as `via: 'policy'` would make *"the workspace policy allows writes here"* and *"a person said yes to everything up front"* the same sentence in the log, and they are different claims about who is answerable. `'standing-grant'` says which.

    Two things this deliberately does **not** do. It does not widen `ToolPolicy.defaultAction`, which is the literal type `'ask'` — that pin is §13's fail-closed default expressed in the type system, and a grant that lives beside the policy leaves it intact rather than teaching the policy language to say "allow everything". And it does not descend: a child session is its own session (§4.3), so it starts asking again. Inheriting would make one decision at the root silently govern work the person granting it had not seen.

    Scoped to allow anything, including writes outside the workspace and network access, which is what makes it useful and what makes it worth the visibility above. §12.1's external-path handling and §13's escalation guard still *record* what happened; what the grant removes is the question, not the account of it.

    *Built.* `CreateSessionInput.standingGrant: boolean` at creation — when the person is present — stored as `Session.standingGrant` and recorded as a `permission.standing_grant` event whose envelope carries the when and the whom; `false` is stored as no grant at all, for the reason a `splitGrant` of zero is. The gate checks it after policy has answered and only an `ask` reaches it, so a policy `deny` and the escalation guard decide exactly as before — the grant settles questions, and a refusal is not a question. Each settled call writes `permission.decided` with `via: 'standing-grant'`, decision `allow`/`once`, and **no actor**, so the transcript can still answer "did I approve this one" the way §17.8's split grant can. Two things building it added: the grant is folded into the projection (checkpoint v3), because a host restart is still the same session and a grant that silently lapsed would re-arm the gate mid-run against the log's own `via` lines; and `prepareChild` leaves it out of the child's input by name, so non-inheritance is a decision in the spawn path rather than an accident of omission.

    Auditing it added two more, both the same shape: a promise held in one process lifetime and quietly not across two. **The grant event carries the policy it was granted beside**, because the session's effective policy is not otherwise durable — a restart rebuilds it from the target's defaults, a loss that used to be fail-closed (a configured `deny` degraded to `ask`, and a person got asked) and would have become fail-open the moment a restored grant answered that `ask` unattended. Restoring the *pair* — the same "travels with the ask" reasoning as Q18's budget — keeps "refusals are untouched" true across a restart, and is tested with a tightened policy surviving one. And **the grant's strength varies with `permissionFidelity`**, so admitting a non-`callback` agent into a granted session says so at admission — a `precomputed-allowlist` gate compiles `ask` to deny before the process starts, reaches the grant only through deny-and-resume rounds, and settles later matching calls inside the CLI's own allowlist where no `permission.decided` is written. The adapter is still not told the grant exists; the orchestrator names the seam while the person who granted is present, rather than at 3am as a stalled run.

20. **Where does MCP live? — decided and built: in the session, never in the app or a CLI's own config.** The obvious homes were both wrong for reasons this document had already written down. An app-level MCP registry is the "preference somebody turned on months ago" that Q19 refuses for permissions — worse, one that quietly widens what every future session can reach. And a CLI's own discovered config is §3.12's reproducibility problem verbatim: what a session could do would depend on whose machine its host runs on, and the transcript would not say.
