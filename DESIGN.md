...
12. **Session templates — recommended: derive them from a session, never author them.** Templates would carry rosters, checklists, a default target, and a model + auth assignment per role.
    ...
    The one field that must not be captured is any credential, and §13's rule already covers it: the template records the auth *mode* and the endpoint id, never a secret.
