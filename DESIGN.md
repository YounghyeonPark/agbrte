...
**The wiring, not the interface, was where the single implementation showed.** `ModelEndpoint.providerId` has been on the record since §3.8 and **nothing read it** — `buildHostRegistry` took one provider and every endpoint, whatever it declared, was answered by that one adapter. Invisible while there was nowhere else a request could go. A `ProviderRouter` now dispatches on the field, with `openai-compatible` as the fallback so an unregistered id behaves exactly as every id behaved before; refusing instead would turn a typo in a field nothing validated into a session that cannot start. That is the shape of what "validated against one implementation is not validated" actually buys: not a broken abstraction, but an assumption that only a second implementation can find.
