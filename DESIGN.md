...
- **A named pipe or unix socket, keyed by `instanceId`.** No port to allocate, collide over, or expose — a TCP listener on localhost is reachable by every process on the machine. Keyed by instance because §5.2 made that the identity of one checkout on one machine, which was exactly one host's scope. **That last clause is what is changing**: a host is becoming one per machine, so the key becomes `machineId` and the record moves to `~/.agbrte/host.json`. Session storage does not move with it — it stays in each workspace's own `.agbrte`, because a folder carried to another machine has to arrive with its sessions, which is what §5.3 exists to protect.
...
**Still one host process per workspace, and that is the remaining step.** The manager is no longer what stands in the way of one host per machine; the socket, the host record and the connection are. They are keyed by `instanceId` today, which is exactly the conflation `machineId` was added to end.
...
22. **Where things live, and what a host owns — the shape settles.** Three questions had been answered separately and turned out to be one. `~/.agbrte` was already the machine's install area (§6.4) while a workspace's store was called `.devagents`, so the two names carried a distinction the code did not actually rest on; the host was scoped to a workspace, so a machine with four checkouts ran four hosts; and attaching asked for a machine *and* a folder, which made a folder a property of the connection rather than of the session that works in it. Settled as one shape: **`~/.agbrte` is the machine** — its host, its socket, its global state, applying to every session on it — and **`<workspace>/.agbrte` is one workspace** — its identity, memory, templates and sessions, chosen when a session is created and loaded as-is if something is already there.
    ...
    **Landed so far, and what has not.** The manager holds a table of workspaces keyed by `instanceId`, `createSession` takes the folder, leases are one table per workspace root, relocation and the inbox marker are per workspace, and a group can span folders one manager holds. The host process is **still one per workspace**: its socket, its record and the app's connection remain keyed by `instanceId`, and moving them to `machineId` is the step that makes attaching ask for a machine and creating a session ask for a folder. That is not a small change and it is deliberately not being made halfway — the app, the CLI, the session protocol and the remote bootstrap all reach a host by computing a workspace's socket, and a half-moved key means two processes able to compute their way to one log, which is the one thing §5.1 does not survive.
