...
| Cost sprawl across providers | surprise bills | one gateway for `api-key`; per-agent/session/day ceilings. **`cost unknown` is now real rather than intended**: every turn carries a cost, three values stay distinct — free, a figure, and unobservable — and one rule for adding them lives in `shared/cost.ts` where there were three private copies. Ceilings themselves are still unenforced |
