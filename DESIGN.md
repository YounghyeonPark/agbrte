...
**The free half also decides the order, which is where that same incident was still live.** A badge tells somebody what they have picked; it does not stop the app pointing them at it. The ready models came out in the endpoint's own order, and on the machine that produced the original report that order is `qwen3:0.6b, smollm2:360m, llama3.2:1b, smollm2:135m, qwen2.5:7b` — so the control opened on the model from the incident and the one that works was fifth. `buildEntries` now ranks within an endpoint on three facts it is already holding, none of which costs a request: a model **watched calling a tool here comes first**, a **declared no on tools sends a model last** — settled rather than guessed, by the asymmetry above — and between them a model **in the catalogue comes before one that is not**, since that file is the only place this project has written down which models are worth suggesting. Nothing is hidden, and endpoints keep the order the host listed them in: ranking across them would interleave two recipients, one of them possibly the network (§13).

**The probed tier is there because the catalogue has a shelf life, and it is the only signal here that does not.** `scripts/model-catalogue.mjs --check` compares the committed list against the registry, so it catches a tag that vanished; it cannot catch one that *appeared*, because `CANDIDATES` is written by hand and there is nothing to ask the registry about a model nobody has named. So the list rots in exactly one direction and stays green doing it — tolerable while it only suggested downloads, not once membership began ordering the models a machine already has. A probe does not rot: it is a fact about that machine, cached by `probe()`, and the picker already pays for it on the model it opens on. Somebody who pulls next year's model and uses it once has ranked it, with no release from us — verified on a live host, where `llama3.2:1b` (in no catalogue, probed) moved above `qwen2.5:7b` (in the catalogue, probed) on the next listing. The remaining half is a **review date in the script rather than in its output**: `verifiedAt` is written by the generator, so a staleness check reading it would be cleared by the one command that cannot fix the problem, while `REVIEWED` can only be moved by opening the file the list is in.

Deliberately unused: **how recently a model was pulled**, which is available (`/v1/models` carries `created`) and ranks a real machine exactly backwards — the newest tag there was `qwen3:0.6b` and the oldest was `qwen2.5:7b`, because people pull small models to try them and install the one they work with once, long ago. Also unused: the catalogue's *order*, which is a download recommendation weighed against what a laptop with no discrete GPU can run. Which of the models already on a machine should be first is a different question, asked where the hardware has already answered it, and reusing one order for both would quietly answer each with the other.
...
| 3 | A thinking **budget** — Anthropic's `budget_tokens` (≥1024, counted against `max_tokens`) | `ReasoningRequest` is `{ mode: 'off' \| 'low' \| … }` | **Built** — the `anthropic` adapter maps effort to a count, clamped to leave room to answer, and is the first thing to produce `reasoningControl: 'budget'` |
...
| 5 | Streaming | `invoke` returns `Promise<ProviderResult>`; both providers declare `streaming: false` | Not yet |
...
- **Tool suite** — `read`, `write`, `edit`, `glob`, `grep`, `bash`, `fetch`, plus `plan`/`update_task` (§10) and `remember` (§5.5). One canonical schema set, degraded per §3.5. These are the same tools `AgentHost` implements for lease enforcement, so there is one implementation, not two.

  `fetch` was written here as `web_fetch` and shipped years later than the rest, and the delay is worth a sentence because it was never about capability: an agent with `bash` could always `curl`. What it adds is legibility, which is §13's actual subject — a shell command is one opaque string to everything upstream of it, so the gate sees `bash`, the transcript records a line of shell, and "which sites did this session read" cannot be answered without parsing arguments. A named tool is a rule a policy can match and a log row naming the URL.

  Its refusals are the substance. **No headers from the model**, because a header is where a credential goes and a tool that took one would be a way for a key to leave with nothing recording it. **No private addresses**, and this is the one that matters most here: §6.2's control channel is on this machine's loopback behind a bearer token, and cloud metadata answers on `169.254.169.254` with credentials for the box, so the hostname is *resolved* and every answer checked — a name check alone is defeated by any service that resolves to whatever you ask it to. **Every redirect hop is vetted again**, since a public URL redirecting inward is the oldest way round a front-door check. What is *not* closed is DNS rebinding: the address is vetted and the request is then made by name. Closing it needs the connection pinned to the vetted address, which is a dependency this project does not have — recorded rather than implied, because a defence with an unrecorded hole is worse than one whose edge is known.
...
- **The two servers this app cannot install still get a first-class answer.** `vLLM` and `NIM` are the reason `target-local` exists, and the app can install neither: vLLM has no native Windows build and its own documentation names WSL2, which needs administrator rights and a reboot; NIM's images are gated behind an NGC account whose key `docker login nvcr.io` authenticates with. The previous behaviour was to offer neither, so somebody with a GPU box typed a vLLM address into the endpoint form, got a connection failure, and was told nothing about the WSL that was never installed — §16's shape again, an unbuilt path failing invisibly. So the entry exists and the button *diagnoses*: `serverReadiness` probes the machine over the same `ProvisionRunner` an install uses and returns an ordered list of what is left, with a `why` on each step the app is choosing not to automate. Ordering carries most of the value — a missing GPU is said alone because it makes every later step pointless, and the NGC key is named even on a machine that already has Docker and the toolkit, so somebody who cannot get one learns it before installing two things rather than after.
- **A question that was never asked must not answer `no`.** The first `serverReadiness` caught every error from the runner and returned `null`, and `null` for the GPU means *there is none*. Run on Windows, where `localRunner` refuses every command by design, it reported **No NVIDIA GPU on this machine** over an RTX 4090 — a confident wrong answer assembled entirely out of questions that were never put. §3.3's three tiers are about exactly this. The fix has two halves: a `RouteRefused` now travels rather than being swallowed, so a machine that cannot be inspected says so in its own words; and `localProbeRunner` runs the read-only probes on Windows, where the provisioning runner is refused because its scripts are `sh`. The Windows port check found the same failure one layer up — a PowerShell one-liner handed to `cmd.exe` was *echoed and exited 0* (§6.2's recorded failure), and the literal `'True'` inside the echoed script text was read as a live server on a machine with nothing on the port. It is now `curl` on both platforms, judged by curl's own exit code, with no shell syntax left in it.
- **The order was enforced for a release before anything could set it.** `nextAfter` answers the chain, `askWithFailover` walks it, and a move writes `model.endpoint_switched` with its reason — and the only endpoint command on the wire was `endpoints.add`, so the order every turn on a machine follows could be changed in exactly one way: by editing `endpoints.json` on that machine, which for the remote GPU box the feature exists for means an ssh session and hand-edited JSON. §16's shape with an extra sting, because the invisible half was the worse one: §13 requires that where source code goes be legible *before* it is sent, and a fallback is a second recipient chosen in advance — somebody whose turns had moved to a hosted API could read the reason in the transcript and could not read the configuration that put it there. v30's `endpoints.chain` writes it and the picker shows it, with the provider on every row.
- **One ordered list, where the file has two fields.** `default` is where a turn starts; `fallback` is the whole order with that default as its first name. A file may legally set a `default` that is absent from `fallback`, and that combination is a trap rather than a feature: `nextAfter` finds the starting endpoint nowhere in the chain and answers nothing, so the one endpoint every turn begins on is the one endpoint a refusal can never leave. The command takes an order and derives both, which makes that state unreachable through a client while hand-editing still reaches everything it did. The command also carries **ids and nothing else** — no URL, no provider, no credential — so reordering can never become a second route to changing what an endpoint *is*.
- **A field went missing at the one layer that copies by name, again.** `endpointChain` was threaded through the agent host's handshake, the advertisement, the supervisor, the session host's identity, the DTO, the fleet snapshot and the picker — and stopped at `sessionServer`'s hello, which rebuilds its identity field by field. Every layer type-checked and the app rendered *Fallback order — not set* over a file that named one. The same shape as v29's `api`, which stopped at a signature, and as `snapshot` in `fleet.ts`, whose comment says outright that a field written onto an entry and not listed there is a field no reader has ever seen. Three instances now, all in the direction CLAUDE.md's first hazard names, and none of them visible to a compiler. What caught this one was a screenshot; what keeps it caught is an e2e that names the first endpoint rather than asserting the panel exists.
- **`defaultReasoning` was the second field on this record declared and read by nothing.** `providerId` was the first, and it took a second adapter to notice; this one took a sweep. The constant in `SessionManager` was the whole of a new seat's effort, so a `defaultReasoning` written into `endpoints.json` changed nothing and reported nothing — §16's shape, in the quietest form it takes, because the field is documented here and in the type and a person writing it has every reason to believe it works. It is read now, and it earns its place on the *endpoint* rather than the session: `max` on a box you own costs time nobody is billed for, and the same `max` on a hosted API is a bill, so somebody with both was changing every new seat by hand. A seat that asks still wins, and a mode that is not one of the six is refused when the file is read rather than dropped — tolerating a typo would reproduce the exact failure the field is being fixed for, and this time the person has been told it works.
- **The decision is at admission, not at invoke, and the shorter route was the wrong one.** The endpoints file is read in the forked agent host (§8), which is also where a request is built — so applying the default there needs no plumbing at all. It would also have left the roster's effort control saying `max` while the request carried something else, since that control shows `spec.reasoning`. A divergence between what a person is shown and what is sent is the thing §13 spends a whole section on, arriving through a field about how hard a model thinks. So the *session host* decides, and the endpoints reach it through the agent host's handshake — which is one more field on a path that has now dropped three.
...
**Cross-provider fallback comes nearly free.** Because rehydration reconstructs context from our own log rather than provider state, an agent stopped by `refused`, `unavailable`, persistent `rate_limited`, or `quota_exhausted` can be restarted on a different provider with its task intact. One caveat stated plainly: **opaque provider-specific reasoning blocks cannot cross a provider boundary** — they are dropped at the handoff and the drop is recorded, so the transcript explains any discontinuity. **Built**, and one step smaller than this paragraph imagines: the move happens *inside* the request rather than by restarting the agent, because the endpoint list lives in the forked agent host and the session host cannot see the chain at all. The order is declared in `endpoints.json` beside `default`, since which of a machine's endpoints to use is a fact about the machine — a GPU box being down is true for every session on that host at once. Two refusals are as load-bearing as the list: a next endpoint is **re-probed** and skipped when it cannot do what the seat is doing (§3.3's spread is exactly why), and `auth` is **excluded** from the moving stops, because answering a missing credential by sending the work to whichever endpoint happens to have one is the unannounced change of recipient §13 forbids.
...
- **A parent with no budget splits into children with no budget.** The absence carries down; nothing is invented. This *refused* for a long time, on the argument that inventing a ceiling would put a number nobody agreed to at the root of a subtree — which is an argument against inventing a number, and was read as one for refusing. The third option was never considered, and the cost of not considering it was two rules meeting: a root may be given a budget at creation and absent means *unbudgeted* rather than zero, since most sessions are a person working and a ceiling nobody chose would stop turns for a reason nobody set — and then that same absence stopped that same person decomposing their work at all. Every split and every workflow run required picking a token figure first, which is a limit imposed by the mechanism rather than by anybody's intent. An unbudgeted tree stays bounded the way an unbudgeted session does, by `maxDepth` and `maxOpenDescendants`, neither of which reads a budget.
...
A child's ceiling is **reserved from the parent's remaining budget at spawn**, so a tree cannot spend more than its root was granted. That survives hosts being plural without anyone owning a total: the debit happens on the parent's host before the child exists, there is no refund path anywhere, and the child enforces a figure it was given (§17.5). Descendants inherit `quotaGroup`, so the QuotaScheduler (§8) throttles an entire tree drawing on one credential. **A session's own spend is bounded too, and that took less than §6.5 says it does.** §6.5 gives ceiling enforcement to the ModelGateway, bundled with a reverse tunnel and credential injection, and §15 records the gateway as deliberately not built — noting that it was worth checking against the deployment first, "if the server is what has model access there is nothing to tunnel". Checking says four of the gateway's five jobs have no work here: routing by `providerId` exists, usage is already on the log, and with no API key in play there is no secret to keep off a remote box. The fifth was the whole gap — **nothing compared the two.** `spent` is folded from `usage` now (input plus output; the cache fields are a breakdown of the input side, not tokens on top of it), and a turn that would start with nothing left parks on `limit_reached { limit: 'tokens' }` before the request rather than reporting it after. **Free tokens are exempt** — a local model bills nobody, and a ceiling exists to bound cost, so charging a budget for one would stop a long local run at a figure chosen for money never spent. Whether a seat is free is stamped onto each `usage` event rather than looked up when the log is read, because which endpoint a turn used is a fact about the moment it ran and an endpoint can be reconfigured afterwards. Per-*day* ceilings and cost enforcement still belong to the gateway and are still unbuilt.

**Which is why a reservation is never released, and why "released on completion" was the wrong half of the pair to keep.** A child's consumption is folded into the *child's* budget from the child's own log, and nothing carries it back — `session.child_result` returns a bounded summary and artifact pointers, not a token count. So a parent cannot know what a finished child cost. Releasing it when a child finishes would let a 100,000-token root spawn 20,000-token children one after another forever: each finishes, each hands its allowance back, and none is ever debited for what it actually used. The sentence a tree is supposed to satisfy would quietly become one about how many children may run *at once*. So a budget grants a **lifetime allowance of child ceilings**, with the consequence stated rather than discovered: running the same workflow twice against one root costs that root twice.
...
**A run on an unbudgeted root checks nothing and reserves nothing**, following §4.3's rule that the absence carries down: there is no total to check the graph against, and no remainder to take from. The node ceilings stay required and stay validated all the same, because they are the author's statement of what each part is worth, and the same document run against a root that *does* hold a grant is checked whole in the usual way — a workflow is written once and run more than once, and a field that appeared only when the first run happened to be budgeted would be a field nobody filled in honestly.
...
**A session's budget was not durable at all.** There was no budget event, so a restarted session had none — and §4.3's `prepareChild` then refused a parent with no budget, which meant **a restarted session could not split**. The whole of this section stopped working at a host restart, and nothing said so. That refusal has since gone, and the durability is if anything more load-bearing without it: a parent that comes back having forgotten its ceiling reserves nothing for the children it spawns next, so the failure changed from a visible refusal into a tree quietly outspending its root.
...
It is the sentence. A release on completion would let a 100,000-token root spawn 20,000-token children serially without limit, each finishing and handing back an allowance it was never debited against, and "a tree cannot spend more than its root was granted" would silently become a statement about concurrency.

**The reason has since narrowed, and it is worth keeping both.** When this was decided the argument was that `spent` is always zero, so `reservedForChildren` was not one term of a budget beside consumption but the *entire* budget. That is no longer true — `spent` is folded from `usage` now — and the conclusion survives anyway, on a smaller fact: consumption is real *per session*. A child's tokens are folded into the child's own budget from the child's own log, and nothing carries the figure back; `session.child_result` returns a bounded summary and artifact pointers by design, not a token count. A parent still cannot know what a finished child cost, so releasing its reservation would still be handing back an allowance nobody debited. Making that releasable is a change to what a result carries, not to the arithmetic.
...
**The tools may write `templates/` and `memory/` and nothing else in this directory.** `confine` — the path check in `tools/index.ts` that keeps every filesystem call inside the workspace — asks whether a path is *inside* the root, and `.agbrte/` is: the store sits in the folder the agent works in. So `write` and `edit` could reach `sessions/<id>/events.jsonl`, `instance.json` and `run/schedules.json`, and an agent editing its own append-only log is the failure that makes every other record here unciteable. `glob` and `grep` have skipped this directory since they existed, on the grounds that "walking one is walking our own bookkeeping"; that writing was never closed with them was an oversight rather than a decision.

`templates/` and `memory/` stay writable, and that is this rule's point rather than a hole in it. They are **content**, tracked for exactly that reason, and §4.4's approval argument depends on an agent being able to propose a decomposition *by writing a file* so that hand-written and agent-proposed documents converge on one artifact reviewed in a diff — skills (§17 Q21) and MCP declarations (§17 Q20) join them under the same argument, the last of those being safe to write precisely because its type has no field a credential fits in. Reading is untouched, since citing the record is the reason for keeping one. And this closes the two tools that take a path: `bash` can write whatever the host user can, where the permission gate is the boundary and `ToolPolicy.defaultAction` is the literal `'ask'`.
...
- **An over-long unix socket path is truncated by `bind`, not refused, so the failure arrives somewhere else entirely.** `sun_path` is a fixed array — 104 bytes on macOS, 108 on Linux — and past it the socket is created under a shortened name while the caller keeps the name it asked for. The first thing to touch that name reports the problem, which here was the `chmod` that locks the socket to its owner: `ENOENT` on a socket that had, as far as anything could see, just been created. A length bug wearing a permissions error, on a listener that had reported success. Found by a macOS CI runner, whose `TMPDIR` is a 48-byte `/var/folders/...` — the machine host's own socket fits there with six bytes to spare, and the comment claiming the limit was "easy to avoid" was resting on arithmetic nobody had done. The check now runs before the bind, where the evidence still exists, and says the length rather than the symptom.
...
- **A sidebar row is a machine, because a host is one process per machine.** It was one row per *workspace*, which is what the fleet's entries are — a binding of one checkout on one host — and while a host was one per folder those were the same thing. They stopped being the same at v21 and the UI kept the old shape: two folders on one build box drew two sections with the same name, the same buttons and different contents. What that produces is not a cosmetic complaint: it was reported as "my remote sessions are gone" by somebody looking at the other row, with the folder holding the sessions no longer attached. So the machine is the row and the folder is a property of the sessions under it, said on each one and only where a machine holds more than one. **Stop** and **Update** are machine acts and now appear once instead of per folder; **detach** lets go of every folder on that machine, since letting go of one checkout and leaving the row behind is not what it means; and the creation form asks *which* folder when there is a choice, because "in this workspace" stopped naming itself the moment the row became the machine.
- **A session gets a folder of its own by default, not as an option.** One session, one folder (§8) was offered as an empty optional field, so the default stayed "another session in whatever folder this host has open" and a machine attached to `~/Desktop` kept putting sessions on somebody's desktop. A rule that is offered is not a rule: the name is filled from the title, the resolved path is shown before anything is created, and clearing the field is how somebody says *in this workspace* — which is a real thing to want, since several sessions on one project is ordinary.
...
**And the forward is a TCP tunnel, which answers the live-screen question without any of this.** `ssh -L 3389` brings a Windows machine's desktop to a local address, `5900` does it where a VNC server is running, and a remote-desktop client then gives hardware-accelerated video, input and the clipboard — which is more than a frame encoder here would ever deliver, and none of it is this project's work. The tunnel always did that; what stood in the way was the session view rendering every forward as a browser link, because §6.8 was written about dev servers. A link to `http://127.0.0.1:54321` is a control that fails on press in front of an address that works the moment it is text, so `shared/preview/protocols.ts` names the ports a browser cannot speak and the row shows those as an address instead. The default runs the other way — an unrecognised port keeps its link, because a dev server listens wherever it was told to and being wrong there costs a tab that does not load.

That was written as *watching the machine is the tunnel; showing the machine to an agent is a capture* — and the tunnel half of it turned out to assume something that was not there.

**The display grab is built, and it is a viewer for the person rather than a still for the model.** What changed the decision was measuring a real machine instead of reasoning about one. An Ubuntu 24.04 box with a GNOME session logged in for six weeks had **no `x11vnc`, no `vncserver`, no `xrdp`** — so `ssh -L 5900` reaches nothing, and the recorded answer ("forward 5900 and use a VNC client") begins with installing a server on somebody's working machine. It also had no ImageMagick, no `scrot`, no `gnome-screenshot` and no `ffmpeg`, and GNOME's own D-Bus `Screenshot` refused with *"Screenshot is not allowed"*, which is GNOME keeping that API for portal callers. What it did have was **`xwd`**, part of `x11-apps`, present and working. So the grab is the path that needs nothing installed over there, which is the property that decided it.

- **It is a live view at a few frames a second, and it says the number on screen.** Measured at 2944×1080, 24bpp: `xwd` takes 0.13s and produces 12.7MB, decoding is 0.01s, and encoding a PNG of it is 0.16s for 430KB. `xwd` has **no damage tracking**, so every frame is the whole screen whether or not anything moved — a ceiling of three to five frames a second that nothing on this side can lift. `tookMs` travels with every frame and the rate is rendered beside the picture, because a viewer silently running at 3fps reads as broken and sends somebody looking for a bug that is a measurement. H.264 was rejected for needing an encoder this project does not have (and `ffmpeg` was absent on the very machine); a codec of our own is a large amount of subtle code to raise a ceiling set by `xwd`.
- **Two commands, and the split is about cost.** `display.list` probes each display and **stops after the header**, so enumerating them costs a few hundred bytes each instead of 12.7MB. It answers with a size per display — or with *why that one cannot be read*, which is the field that took the most thought: a display whose X cookie the host does not hold is a real screen with a fixable problem, so dropping it would say "no screen" (§3.3) and offering it as ready would be a control that fails on press (§3.5). It is listed, disabled, and the reason is on screen. Likewise `tool: null` beside a non-empty list means *there is a screen and this host cannot read it yet*, which is one `apt install x11-apps` — an empty list would have sent somebody to debug their X server.
- **Pull, one frame per request, and that is the whole of the backpressure.** Nothing is pushed. The viewer asks for the next frame when the last one lands, so a slow link produces a slow view rather than a queue of 430KB payloads on the connection that also carries the transcript. A `setInterval` would have been fewer lines and would have kept firing into a link that had stopped keeping up — and a timer anywhere but in the visible pane would keep pulling whole frames off somebody's desktop after the window showing them was closed.
- **`xwd`'s format is decoded here, which completes a pair rather than adding one.** Nothing on that machine could read what `xwd` writes — Pillow was installed and cannot open it — so the choice was a dependency on the far machine or a decoder on this side, and §12.1 already writes PNG by hand for redaction's sake. The two fields that actually vary are `bits_per_pixel` and `bytes_per_line`: a padded row read as an unpadded one shears the picture a little further on every scanline, which looks like a broken capture rather than a broken reader. The channel masks are **read rather than assumed**, because hardcoding the BGR order this machine reports would silently swap red and blue on the first machine that disagrees — and swapped colour looks like a rendering opinion, not a bug.
- **A read, and deliberately not a tool.** No §13 gate, for `files.list`'s reason: §13's gate covers what a *model* asks the app for, and there is no agent on this path — this is a person looking at a machine they are already running a session on, and prompting them to approve their own click is the theatre §13 warns about. Nothing is logged, stored as a blob or attached to a turn. Handing a *model* the whole screen of somebody's workstation is a different decision and would need its own name at the gate, so there is no `display` tool. There is no input either: no pointer, no keyboard, no clipboard. **VNC remains the better answer for anything interactive**, and where a VNC server does exist the tunnel and `shared/preview/protocols.ts` still point at it.
- **Not offered to a stranger.** `display.list` and `display.grab` are absent from `PUBLIC_CHANNELS`, and the reasoning nearly carried too far: `files.list` is public because the *workspace* bounds it, and a display is bounded by nothing — it is whatever happens to be on the screen of the machine serving the page, including the mail client of whoever set the demo up.
- **A compositor it cannot see is named rather than left as a black picture.** `xwd` reads an **X** display, and under Wayland that is XWayland — whose root is not the compositor's output, so the grab returns black while the desktop is plainly there on the monitor. The host looks for a socket under `/run/user/<uid>/` and the viewer says so *before* the frame arrives. A filesystem fact rather than `XDG_SESSION_TYPE`, for the reason the X displays are found the same way: a host started from a non-login shell over ssh inherits almost none of a desktop session's environment. And a **caution, never a refusal** — some compositors do put something on the XWayland root, this host cannot know which, and hiding the display on a guess would withhold a view that might have worked (§3.3).
- **The portal path is built, and the consent it needs is a feature rather than an obstacle (v38).** Measuring the same machine again settled which of two portals this could use. `org.freedesktop.portal.Screenshot` is **not usable**: with `interactive: false` it refuses in 0.01s, and with `interactive: true` it raises a dialog per still — an unattended refusal and a dialog-per-frame are both the opposite of a viewer. `org.freedesktop.portal.ScreenCast` **v5** carries `persist_mode`, which is the only shape in which this feature can exist at all: consent **once**, and a `restore_token` that restores silently from then on. So the compositor's screen becomes a row in the same list as the X displays, and the frames come off a PipeWire node through GStreamer — `pipewiresrc ! videoconvert ! pngenc`, both already on the machine, which is `xwd`'s own property and the reason neither was vendored.
- **The consent is asked once, by its own command, and nothing routes around it.** `display.approve` is separate from `display.grab` because they are separate things to happen: a grab is a fraction of a second, and this puts a dialog on somebody's monitor and waits minutes for them to walk to it. Folded together as a flag, the viewer would have had no honest label for the button and no way to show a wait as a wait rather than as a slow frame (§4.1). A program that could silently read a desktop is precisely what Wayland exists to prevent, and this does not try to be one. The token that comes back is a **machine fact** and lives in `~/.agbrte` (§5.1) — never a workspace, a template or the log; it is not a §13 credential, since it grants nothing to anybody who is not already on that machine's session bus.
- **"Nobody has said yes yet" needed a third state, because the two that existed were both wrong.** `DisplayInfo` had *ready* and *unreachable*. A desktop awaiting consent as **ready** is a control that fails on press (§3.5) — the grab raises a dialog on a screen nobody may be sitting at and times out. As **unreachable** it is a `no` about a machine that is one click from working (§3.3). So `needsApproval` carries the sentence, the pane shows the question and a button that asks it, and the watch control is not rendered at all until there is something to watch. Measured on a real machine: two four-minute waits ended with the dialog still up, unanswered — which is exactly the case the third state exists to describe rather than to report as a failure.
- **The cast is held between frames, and let go when nothing is pulling.** Even with a token, `Start` is three round trips to the portal plus a PipeWire node being set up — most of a second, which per frame would put this *under* the X path it exists to beat. So one session serves many frames. It is dropped after thirty idle seconds for a reason that is not about memory: GNOME shows a running screencast in its status area, and a session left open after the window watching it was closed would be this app appearing to record a desktop nobody is looking at. A frame that fails drops it too, since a node that stopped answering is usually a session the compositor ended, and holding the dead handle would make every later grab fail the same way for ever. **And it is closed twice over**, because the ways a host dies do not all run cleanup: `stop()` releases it beside the shells it already sweeps, and the helper itself blocks reading its stdin and ends when that pipe closes — which is what a host killed with `-9`, a crash or a machine going down look like from the far end. An orphan here is not a stray process; it is a screen-sharing indicator lit on somebody's desktop until they reboot.
- **`display.approve` is the sharpest thing kept from a public host.** The whole `display.` prefix is refused, so it needs no separate argument — but it is worth naming, because it does not read a screen: it raises a dialog **on the serving machine's own monitor** and waits. A visitor who could reach it could make a stranger's desktop ask to be shared.
- **What this repository cannot verify.** A real X display, a real cookie and a real desktop session exist only on the user's machine. The decoder and the `xwd` driver are tested against built dumps, and the end-to-end run proves the whole path by asking a **Windows** host — which genuinely has no X display — and getting that answer back through every layer. A frame off a real GNOME session is the user's to confirm. **Wayland is now the larger gap, and it is a specific one.** The portal conversation is verified as far as the dialog — `CreateSession` and `SelectSources` returned success with exactly the options the helper sends, and `Start` was seen to raise `"Share Screen"` by name. **Nothing past that dialog has ever run.** Nobody has approved one, so no `restore_token` has come back, no PipeWire node has been opened and no frame has been read through one; everything downstream of the consent is tested against an injected `spawn` and has never met a portal. That cannot be closed from here — it needs somebody standing at a Wayland machine — and `docs/status.md` says so rather than letting a green suite imply otherwise.

**The palette is a choice, and a narrow one.** Four themes ship — warm, graphite, midnight, slate — and what they may vary is smaller than it looks. The reasoning above is functional rather than decorative, so every one of them keeps exactly one accent, keeps amber asking and red warning (§4.1 requires a pause and a failure never to blur), and keeps the ground off blue: the interface *was* Tokyo Night and moved away because a blue-tinted ground makes an amber signal read as a colour clash where a neutral one makes it read as a lamp coming on. A theme varies the temperature of the neutrals and the distance between surfaces, which is enough to change how the program feels and not enough to change what its colours mean.

- **The rule became a gate rather than a paragraph.** `themes.test.ts` walks every palette and checks each colour that carries text against that palette's own ground, with floors set where the shipped palette already sits rather than at the bare standard — a new theme that merely scraped AA would pass WCAG and still be the dingiest thing in the list, which is the complaint that started the work. It also checks the surfaces stack in the order they are named, that the two signals stay apart in hue, and that a border can be found against the panel it edges. One palette checked by hand drifted to 4.30:1 once; four checked by hand is that bug waiting to happen four times.
- **The choice is the client's, not the host's.** It lives in `localStorage` beside the remembered agent default, for that field's reason: a host owns the log, the queue and the gate, and does not own what colour somebody likes looking at. A session driven from a laptop and a phone at once should not force one of them into the other's choice.
- **Applied by setting variables, before anything renders.** Tailwind's `@theme` emits the tokens on `:root` and every utility reads `var(--color-bg)`, so a theme is thirteen custom properties on the root element rather than four copies of a stylesheet. `startTheme()` runs in `main.tsx` rather than in an effect, because a theme applied after mount is a window that shows the default for a frame. An inline script in `index.html` would be earlier and is not available: `script-src 'self'` is holding back model output that reaches the DOM, and is not worth relaxing for a frame. The window's own `backgroundColor` still flashes the default, which `main.ts` already records and which would need main to read a preference that lives in the renderer.

Platform notes: macOS needs Screen Recording permission — check `systemPreferences.getMediaAccessStatus('screen')` before the first capture and route the user to System Settings rather than producing a black frame. Windows needs none. Wayland requires the portal path for *client* capture, which is still not built. For the **remote display grab** it now is: `xwd` sees only XWayland's root there, so the host offers the compositor's own screen through `ScreenCast` instead, at the cost of one consent on that machine. The probe that decided the original feature found Xorg on vt2, which is the case the `xwd` path was measured against.
...
**"Which endpoints an agent used" became a question the log could not answer, and failover is what made it one.** While a seat had one destination, the seat *was* the answer — `agent.created` carries the `ModelRef` and that is where every turn went. Failover changed the fact without changing the record: every turn starts where the seat was pointed (see §3.9), so a session whose GPU box is refusing bounces between two providers turn by turn, and the only account of that in the log was `model.endpoint_switched` — which records moves and cannot record returns, because there is no move to record. A reader could see *moved to the hosted API* once and had no way to learn whether the next four turns went back, or that the fifth did not. So the `usage` event carries the endpoint that answered it, which is the same reasoning that put `free` there: which endpoint a seat used is a fact about the moment the turn ran, not something to derive at read time from a configuration that has since changed. It folds into a per-agent set in first-use order — the question is *whether any of this code left*, and one turn out of two hundred is the same answer as all of them — and it surfaces in three places, each the moment somebody can act on it: the roster says *also sent to X* when the set holds anything the seat does not name, an export names every provider the conversation reached, and neither says anything at all where there is no endpoint to name. Absent means *no endpoint* — the echo runtime, a vendor CLI with its own model and sign-in — and never an unknown one.
...
| 5 | Remote execution | **2nd** | criteria met, with one honest edge: the detached-remote mechanism is verified against the real server, but the *model* half of "an agent on a GPU box using that box's own model server" is untested because that box has none. ModelGateway still not built, and checking it against the deployment — as this table said to — found that four of its five jobs have no work here: the token ceiling it was holding hostage is enforced without it (§4.3), and the tunnel, the credential injection and the token auth answer a problem a keyless deployment does not have. What stays unbuilt with it is per-day and cost enforcement |
| 2 | Persistence hardening | 3rd | **done** — identity, `PathCodec`, `rehydrate`, blobs, detection, and the notice |
| 3 | Three-shape proof | 4th | **done** — runtime axis validated (four candidates), and the provider axis now has a second, natively different wire format in the `anthropic` adapter. It found nothing wrong with `ModelProvider` and one thing wrong beside it: `ModelEndpoint.providerId` was on the record and unread, so a `ProviderRouter` now dispatches on it |
...
| 9 | Workflows | 9th | **done** (§4.4) — types and validation, a home for templates, the graph view, the editor, and a runner that spawns nodes as their dependencies finish. A run survives its host restarting. Building that closed two older gaps of the same kind: a resumed session was hardcoded as a root, and a session budget was not durable at all — so before this, no restarted session could split |
...
**This does not contradict Phase 3's "deliberately early" argument**, which is worth being precise about because it reads like it should. That argument is that an abstraction validated against one implementation is not validated, and it is satisfied **for the runtime axis**: four candidates run the contract suite. **It was not satisfied for the provider axis, and this document said it was.** `ModelProvider` had exactly one implementation, and only one of those four candidates touches it at all, so calling the gap *breadth* rather than *validation* conflated the two axes. What Phase 3 needed was a natively different wire format — a second OpenAI-shaped endpoint proves nothing, being the same adapter pointed at another URL.

**Now satisfied, and what it cost is the interesting part.** The `anthropic` adapter is that second wire format, and the boundary held: nothing in `ModelProvider` had to change. Four things bent, each of them a fact about the *first* adapter's dialect that had been read as a fact about providers. `ProviderMessage` has a `tool` role and this API has none — a result is a block inside a user message, and results answering one turn must arrive together, so the adapter coalesces. `system` arrives by two routes that had never disagreed. `reasoningControl: 'budget'` had sat in the union unproduced since the table above was written. And `cacheWriteTokens` finally has a producer, having been split from `cacheReadTokens` on an argument no implementation could yet demonstrate.

**The wiring, not the interface, was where the single implementation showed.** `ModelEndpoint.providerId` has been on the record since §3.8 and **nothing read it** — `buildHostRegistry` took one provider and every endpoint, whatever it declared, was answered by that one adapter. Invisible while there was nowhere else a request could go. A `ProviderRouter` now dispatches on the field, with `openai-compatible` as the fallback so an unregistered id behaves exactly as every id behaved before; refusing instead would turn a typo in a field nothing validated into a session that cannot start.

**And the same assumption was in a second place, which the first fix hid.** `loadEndpoints` built every `ModelEndpoint` with `providerId: OPENAI_COMPATIBLE_PROVIDER_ID` as a constant — so an adapter and a router that dispatches on the field were both perfectly correct and completely unreachable, because nothing could produce an endpoint that named anything else. The abstraction was validated and had no way in. `endpoints.json` takes an `api` field now, refused at load if it names nothing this host speaks: the router tolerates an unknown id because it meets files written by older builds, and the loader refuses one because it holds the whole set and can name the remedy — and a typo falling back to the default would send source code to a vendor the user did not choose (§13). The field is deliberately not called `provider`, which is taken by the §13 disclosure label beside it and is free text; a routing field that looks like a label is one somebody fills in with a label.

That is the shape of what "validated against one implementation is not validated" actually buys: not a broken abstraction, but assumptions that only a second implementation can find — and one of them sat behind the other.
...
5. **Should a child ever run in a different workspace than its parent's repo? — yes, and it is built, but the depth cap is still open.** The schema always allowed it and the cross-repo case needs it; the objection is that a tree spanning four `.agbrte/` directories is hard to reason about and any one of them can be unreachable, so capping cross-workspace children at depth 1 is worth deciding.
    ...
    **Now exercised against two hosts rather than argued about.** The three steps run between two hosts holding their own workspaces, managers and sockets, over the platform socket rather than the in-memory channel every other host test uses — which matters because that channel hands the peer the same object, so nothing that lives in the *encoding* is visible through it. That is how a key set to `undefined` survived review: present in memory, gone in JSON, and the shape a socket carried was never the shape a test saw. What two hosts in one process still cannot show is the pair disagreeing — two builds, two versions, a field one side has never heard of — and two spawned processes are blocked on something real rather than on effort, since a split begins with an agent calling `propose_split` and that has no command on the wire by design.
...
The creation form now carries it (`McpServers.tsx`): name, command, arguments and environment, in the per-host new-session form, because that is the moment "what may this session reach" is being decided by the person deciding it. Env values are typed masked, dropped from renderer state the instant the create is sent, and never rendered back from a session — the same rule that keeps them out of `mcp.attached`. What attached is shown above the seat rather than beside it, since the servers are named before a model is chosen, and `mcp.failed`'s reason takes the exact place the tool names would have had.
    ...
    **The machine can hold the value now** (`secrets.list`/`set`/`delete`, v33). The first asymmetry above is a rule about files that *travel* — §13's own words are the log, the events, the UI, a template or a reply — and `~/.agbrte` is none of those: it is outside every workspace, in no repository, and it has held `endpoints.json`'s `apiKey` since §3.8 under exactly this argument. So a project can now *name* what it needs and the machine can answer, which is what stops a credential being retyped into every session that wants the same server.

    What that changes is the **second** asymmetry, and deliberately: a host that holds the value can rebuild a connection the log cannot. That was recorded as a cost rather than a virtue — restarting a host takes a running session's tools permanently, and the cure was somebody retyping a whole server definition — and buying it back is the point. The price is the sentence the endpoint form already says out loud: anyone who can read that home directory can use the key. **`secrets.list` answers with names and there is no command that reads a value back**, because a name is what somebody needs in order to know what is stored and to remove it, while a value's only caller is the spawn on the machine holding it; the two readers are separate functions rather than one with a flag, since a flag is one typo from a reply with a key in it. Storing and forgetting are **writes**, gated like `endpoints.add` — §7's read-only role exists so a phone can watch a build box without driving it, and a client that could store a credential there could point that box at an account nobody on it owns. Listing is *not* gated, which is the other half of the same thought: knowing a machine holds `SEARCH_API_KEY` says nothing about what it is, and a watcher who cannot see the names cannot tell a missing key from a broken server.

    **And a project can declare one** (`mcp.project`, v34). Q20's answer put the declaration in the creation form, which is right about *when* — and left the project itself mute. A repository that uses a search server said so in a README paragraph if somebody wrote one, and every colleague retyped a command, its arguments and a key. So a server can be a file in `<workspace>/.agbrte/templates/` beside the workflows and the skills, under Q12's argument that a new colleague gets project facts by cloning.

    **The declaration has nowhere to put a credential, and that is the type rather than a filter.** It carries `envFrom` — the variable the server wants, mapped to the *name* the machine stores it under (v33) — with both halves validated as environment-variable names, so a pasted key fails the name rule and a key that somehow passed it would be used as a lookup and resolve to nothing. Validating an `env` value against `${…}` would have been the obvious alternative and is much weaker: a refusal on read stops a bad file being *used*, not being *committed*, and by then the secret is in every clone. `workflows.ts` reached the same conclusion for the same reason — "a `Workflow` has no field a secret fits in, no `env`, no headers, no command". One file per server rather than one map, so adding a server is a new file in a diff rather than a line inside one, which matters here more than anywhere else in that directory because what is being reviewed is what a machine will be asked to execute.

    The read joins two facts owned by two places: the workspace says which names a server wants and only the machine says which it holds, so `missing` is computed in the host — §8 puts both there — rather than by a client keeping a second copy of the rule. **Still nothing attaches itself.** This is a list a person picks from, exactly as the form was, and Q20's refusal of an app-level registry is untouched: one project's declaration, travelling only to people who have that project.

    **And a declaration becomes tools** (`session.attachProject`, and `projectServers` on `session.create`, v35). Both name a declaration **by id**, and that is forced rather than chosen: the obvious alternative is a client reading `mcp.project`, filling in the environment and sending an ordinary `session.attachMcp` — which means the values travelling out so they can be sent back, undoing the reason they are on the machine at all. So the resolution happens in the host, the one process holding both the file and the secrets. What reaches `SessionManager` is the `McpServerConfig` it has always taken, so a session equipped this way is indistinguishable from one somebody typed, and `mcp.attached` keeps meaning one thing.

    Every refusal happens **before** anything is spawned or created, and each names what to fix. A missing secret is refused by *name* — that name is the list a person has to fill in, and "could not start" would send them to a broken server instead of an absent key. An id naming no declaration refuses a whole `session.create` rather than leaving a session behind that was asked for a server and did not get one, which is the rule `createSession` already follows for a duplicate id.

    **And a resumed session gets them back** (v35). This paragraph's second asymmetry said "a resumed session does not silently reconnect: the log deliberately cannot rebuild what it deliberately does not hold", and that cost was real — a host restart took a running session's tools permanently. For a *declared* server both halves survive a restart now: the command is in the workspace file and the value is on the machine, so resume rebuilds it. A hand-typed server is unchanged and still cannot come back, which is the same sentence with the same reason behind it.

    Which servers to bring back is read from the log — `mcp.attached` and `mcp.failed` both count, since a server that did not start is still one this session was asked to have and a resume is exactly when another go is worth taking. Whether one is a declaration is answered by the workspace **now**, with no marker on the event: a durable claim about the past would be answering a question the present can answer, and if this workspace declares `search` then `search` here *is* that declaration — the identity rule `attachMcp` already enforces when it refuses two servers under one id. A failure is recorded as `mcp.failed` and never thrown, because a resume that fell over on a rotated key would take the transcript with it, and "why can it not search any more" has to be answerable where the question is asked.

    **And the app knows a couple of them** (`shared/mcp/catalogue.json`, `mcp.declare`, v36). Using a well-known server meant hand-writing JSON, which is a barrier with nothing to do with the decision being made. So there is a catalogue, in `models/catalogue.json`'s shape and with its `verifiedAt`, because both are claims about somebody else's registry that go stale without anybody here noticing.

    **It is a catalogue and emphatically not a registry.** Nothing in it is on, nothing attaches itself, and picking one *writes the file* — the same artifact somebody types by hand, tracked, in a diff, and gone to a colleague on the next clone. A catalogue that configured something invisible instead would be the app-level registry this answer refused, wearing a button. The write refuses an id that is taken rather than replacing it, for `addEndpoint`'s reason: the id is the prefix of `mcp__<id>__*` and swapping the command under one silently changes what every rule pointing at it now permits.

    **Every entry says what it costs to start**, beside its name: `no account`, or a free signup and a key. The first version of this shipped one search entry whose *free* tier wanted a credit card and said nothing about it, so a two-line list read as a recommendation — and a catalogue that hides the cost is worse than no catalogue, because the person finds out after picking. Search now leads with the option needing no account at all, a SearXNG instance, and the paid one is gone rather than demoted.

    Driving a locally installed browser at a search page was the obvious alternative and is refused: it is against most engines' terms, the throwaway profile `capture/headless.ts` rightly uses for a screenshot is exactly what collects consent pages and bot checks, and the agent cannot tell a bot check from a result — a silent wrong answer, which is the failure this project spends §3.3 and §3.5 avoiding.

    Three entries, and the shortness is the design. `read`, `write`, `edit`, `glob`, `grep`, `bash` and `fetch` are built in, so a filesystem server or a fetch server would be a second, worse copy of a tool that is already there and already gated (§3.7). What is left is the work needing somebody else's account: searching the web, and reaching a forge. **This is also the answer to "why is there no search tool"** — search is a vendor, an endpoint and a key, and an MCP server for whichever one you use is a file rather than a decision baked into this program.

    **And on a session that already exists** (`session.attachMcp`, v24). "Named when the person is present" was read as *named at creation*, which made one of the two asymmetries above into a dead end: a resumed session cannot reconnect — the log does not hold the values — so before this, restarting a host cost a running session its tools permanently, and the only cure was to abandon it for a new one. The command is the same act said later: same id rule, same `mcp.attached` at the moment it happens, same `mcp.failed` in place of the tools. Two questions had to be answered before it could exist, and both have one answer — **the spec is built per turn**, so a server attached mid-turn changes nothing under the running model and is declared on the next one (`groupPeers` already works this way), and the log cannot disagree with the app because the event is appended where it happened rather than implied from creation. A duplicate id is **refused, not replaced**: the id is the tool name a policy rule matches on, so swapping the process under `mcp__search__*` would leave every rule pointing somewhere new with nothing recording the change. Still no app-level registry, still nothing inherited by a child, still nothing that survives a restart on its own — what moved is who may say "this session, this server, now".
...
21. **Skills — Q20's answer again, minus the asymmetry.** §17.1 left "progressive instruction loading" as the first thing that would test the harness rule, and it did: a skill is instructions deferred — the model sees `skill__<id>` in its tool list with the description, and pays for the body only when the work calls for it — so it is a *tool*, and it rides the session-tool channel Q20 built with the harness untouched.
    ...
    Three decisions shape it. **Loading is allowed by an explicit rule, not a widened default**: the body is text the person supplied at creation, and gating them from reading their own instructions back would be a prompt per paragraph — so `createSession` writes `{tool: 'skill__<id>', action: 'allow'}` into the session's policy, where it is inspectable, a user `deny` still outranks it (deny → ask → allow), and every load writes `permission.decided` via `'policy'` naming the rule; `defaultAction: 'ask'` stays pinned. **An oversized body is refused at creation, never truncated in flight** — Q7's 8,000-character cap holds for every tool output, and a skill that arrived cut off would silently teach half of what its author wrote, so the refusal names the number. And **a skill survives a restart when an MCP server deliberately does not**, because the difference is the credential, not the mechanism: `skill.attached` carries the whole body the way `session.brief_received` carries the brief, the projection folds it (checkpoint v3), and resume rebuilds both the tools and their allow rules from the log. The log can hold the truth entire here, so it does. Children inherit neither skills nor servers: a child session is its own session (§4.3) and its brief — not an ambient inheritance — is how a parent passes context down. **And now from the workspace, which is where the retyping stopped** (`skill.list`, v32). A skill could only be typed into the creation form, so "how we write commit messages in this repo" was a project fact re-entered by every person into every session — exactly what Q12's "a new colleague should get this by cloning" argues against, and this paragraph already said belonged there next. A skill is now a file in `<workspace>/.agbrte/templates/`, beside the workflows and the session templates and tracked with them, and the form lists what the workspace holds for the person to tick.

    Three things keep that from becoming the registry Q20 refused. **Nothing attaches itself**: the list is offered, and a session gets what somebody ticked — there is still no discovery, no default set, and no ambient inheritance by children. **The body travels at creation, not a reference to it**, so `skill.attached` carries what the session was actually given and a file edited tomorrow does not silently change what ran today — the same property the brief has. And **the read has no write beside it**: `workflow.save` exists because §4.4 wants an agent able to propose a decomposition, and nothing proposes a skill. A save command would be a way for a session to author its own instructions, which is the loop this answer's "explicit, inspectable rule" is written to keep out.

    Markdown rather than JSON, unlike a workflow: a workflow is a graph and JSON is the honest spelling of structure, while a skill is prose, and the reason it is in a tracked directory at all is that a diff is where it gets reviewed. `"instructions": "Line one
Line two…"` is one line in a diff for a whole rewrite. The frontmatter is read without a YAML dependency, and a file that `createSession` would refuse — a bad id, no description, a body over Q7's cap — is a row in the list with its reason and a tick that cannot be pressed, because the alternative is a control that fails after a session has been made.

    **Unbuilt, deliberately:** any tool-triggered skill discovery from disk, which would be the app-level drift Q20 refused wearing a filesystem path, and carrying skills in a session *template* (Q12's own file, which drops them today along with the MCP servers — the next place this argument lands).
