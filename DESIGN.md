...
12. **Session templates** carrying rosters, checklists, a default target, and a model + auth assignment per role. Likely Phase 8.
13. **Memory curation** — should agents prune project memory autonomously or only propose deletions? Leaning propose-only; silent memory loss is very hard to debug.
