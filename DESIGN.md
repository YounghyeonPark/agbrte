...
5. **Should a child ever run in a different workspace than its parent's repo? — built, and the answer came from splitting the spawn rather than from owning anything.** The schema allows it and the cross-repo case needs it, but a tree spanning four `.devagents/` directories is hard to reason about and any one of them can be unreachable. Possibly cap cross-workspace children at depth 1.
    ...
    *Built.* `prepareChild` decides and changes nothing, `createSession` takes the position and brief a child inherits, `recordChild` commits — and the fleet runs the three against two hosts. No two-phase commit, for the reason above: the debit lands after the child exists, so a creation that fails on the far host leaves nothing behind on the near one.
    ...
    What is still not built is the bubbling, deliberately: `rollUp` walks one host's map, so a cross-host parent does not yet learn that its child is blocked. It is a derivation over sessions the fleet already holds and needs no new command — but writing it before a cross-host child could exist would have been code for a case that could not occur, and now that one can, it is the next thing rather than a speculative one.
