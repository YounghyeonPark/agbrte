...
**A session's budget was not durable at all.** There was no budget event, so a restarted session had none — and §4.3's `prepareChild` refuses a parent with no budget, correctly, which meant **a restarted session could not split**. The whole of this section stopped working at a host restart, and nothing said so.
