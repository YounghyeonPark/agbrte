...
**The version bump stranded every running host, and that is the check working.** `SESSION_PROTOCOL_VERSION` went to 2 and the in-repo tests immediately failed against a detached host built from the previous commit — `host speaks v1, this app speaks v2`, refused at handshake exactly as §6.4 intends. Worth recording because this is the first **purely additive** bump: two new commands, no field moved, and an older host would have served every existing command correctly. The equality check is coarser than the change required, and a newer app now costs a restart of every detached host over a feature the user may not use. Left as-is rather than replaced with feature negotiation, which is a real design and should not be introduced as a side effect of adding a command — recorded in §17 instead.
...
16. **The protocol version check is all-or-nothing, and the first additive bump showed it.** `SESSION_PROTOCOL_VERSION` is compared for equality at handshake, which is right when a field moves — a mismatch should be refused before a command half-works. §6.7 added two commands and moved nothing, and an older host would have served everything else correctly; instead every running detached host was refused until restarted, over a feature its user may never invoke. The fix is a minimum-supported version, or capability negotiation in `welcome` so a client can find out that `blob.put` is unavailable without losing the connection. Not built, because it is a real design and adding it as a side effect of adding a command is how protocols acquire two versioning schemes.
