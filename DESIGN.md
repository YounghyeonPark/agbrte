...
**Not built:** the host offering ports on its own. Detection runs where the session runs, so surfacing it needs a session-protocol command; today the port is typed in. The control is shown for remote sessions only, because a local dev server is already on `localhost` and a button that does nothing visible teaches people the feature does nothing.
