...
- **Context management — designed, not built.** The intent stands: no server-side compaction to lean on, so compact at a high-water mark by calling the **same `rehydrate()`** used for moved workspaces (§5.4), one code path exercised constantly so the durable path can't rot. None of it exists. Nothing emits `agent.compacted`, so the statistic the reducer folds is always zero, and `compactionSizes()` computes a payload no caller asks for.

  The blocker is the seam the "one code path" claim runs into: `rehydrate()` needs the store, and `RuntimeContext` — all a runtime is given — has no route to the log. Closing it means either a compaction callback on the context or the session host driving compaction, and that is a decision, not an oversight to tidy. Until then a long session grows its message array until the provider refuses it, which arrives as `context_overflow`.
...
18. **Who compacts, when a runtime cannot reach the log? — open, and the one thing blocking §3.7's context management.** The design says the harness compacts at a high-water mark by calling the **same `rehydrate()`** used for moved workspaces, so the durable path is exercised constantly. Nothing is built, and the reason is a seam rather than an oversight: `rehydrate()` needs a `SessionStore`, and `RuntimeContext` — everything a runtime is handed — has no route to the log. That is deliberate. §17.1's rule is *the harness may decide, and may not own*, and the log is the thing it most conspicuously does not own.
    ...
    The callback is the better fit for the rule that produced every other answer here, and it is a genuine addition to the context contract, so it is recorded rather than assumed. Until one exists, a long session grows until the provider refuses it; that arrives as `context_overflow`, which §4.1 now surfaces as *needs a person* instead of leaving the session displayed as working.
