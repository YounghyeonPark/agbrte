...
9. **Rehydration is also the in-session compactor.** `AgbrteHarness` compacts by calling the same function. Keep it one code path — that is what stops the durable path from rotting.
